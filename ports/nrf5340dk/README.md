...
`make dfu-key` is not optional under the default `DFU=1`, and it is once per
clone rather than once per machine: the key is gitignored, so a fresh clone or a
new git worktree refuses to build until it has one. The same key serves the
DWM3001CDK. `DFU=0` needs no key, because that layout has no bootloader.
...
| `DFU=0` | opt out of MCUboot plus Matter OTA, **on by default** | same run: MCUboot chains to a fully working lock. The OTA Requestor itself is still unexercised |
...
**The signing key is this checkout's, and the build proves it.** `DFU=1` used to
leave `CONFIG_BOOT_SIGNATURE_KEY_FILE` at the upstream default,
`bootloader/mcuboot/root-ec-p256.pem` in the fetched workspace, which is a
private key published in MCUboot's own repository: anyone could sign an image
that bootloader would accept. It is now set to `firmware/keys/mcuboot_ec_p256.pem`,
the same per-checkout key the DWM3001CDK uses, generated by `make dfu-key` and
gitignored. `firmware/keys/README.md` covers rotation and what losing it costs.

There is no second key mechanism, which is the point. `scripts/check-signing-key.sh`
holds MCUboot's list of published demo keys and the four refusals (nothing
configured, a demo basename, a relative path, a path that is not there), and both
ports run that one file: `firmware/sysbuild.cmake` for the CDK, and
`scripts/build-nrf5340dk.sh` for this board, which cannot have a `sysbuild.cmake`
of its own because its application is a fetched upstream tree.

**And it checks the artefact, not the flag.** After the build, the script reads
`CONFIG_BOOT_SIGNATURE_KEY_FILE` back out of `build/nrf5340dk/mcuboot/zephyr/.config`
and fails if it is not the key that was asked for. That is the same lesson as the
LTO guard below: a `-D` flag sysbuild declines to honour leaves a build that
succeeds while meaning something else, and here the something else is a
bootloader that boots anybody's firmware. MCUboot's own objection to the demo key
is a `message(WARNING)`, which is indistinguishable from silence in this log.
