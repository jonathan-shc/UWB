 ...
 * modules/woz_dfu/include/woz_dfu_rx.h is the receiver the transport feeds, and
 * the applier runs in MCUboot. What is here is the one call that puts this
 * port's transport in front of it.
 */
#ifndef WOZ_FREERTOS_DFU_H
#define WOZ_FREERTOS_DFU_H

/**
 * Register the update channel and arm the button that opens its window.
 *
 * Call before woz_freertos_nimble_host_start() -- the same ordering rule as
 * every other NimBLE registrant, because the services are added inside that
 * sequence. In practice that means before aliro_reader_start(), which is what
 ...
 * woz_dfu_window_open() has been called, which SW2 does.
 */
int dfu_ble_start(void);

#endif /* WOZ_FREERTOS_DFU_H */
