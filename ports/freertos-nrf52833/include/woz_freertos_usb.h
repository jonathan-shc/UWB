 ...
 *
 * Never brought up on an operational boot. woz_freertos_usb_start() is called
 * only in provisioning mode, so a lock in a hallway pays the flash for this and
 * never runs a USB interrupt.
 */
#ifndef WOZ_FREERTOS_USB_H
#define WOZ_FREERTOS_USB_H
 ...
 * enumerating, not that a host has opened the port; woz_freertos_usb_ready()
 * answers that.
 *
