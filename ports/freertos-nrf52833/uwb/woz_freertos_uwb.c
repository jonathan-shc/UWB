/*
 * The port's bring-up of the DW3110. woz_freertos_uwb.h says why it exists;
...
int woz_freertos_uwb_start(void)
{
	...
	if (rc != 0) {
		 ...
		 */
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, UWB_TAG, "DW3110 bring-up failed (%d)", rc);
		s_ready = false;
		return rc;
	}

	woz_freertos_log(WOZ_FREERTOS_LOG_INFO, UWB_TAG, "DW3110 up, PHY ch%u code%u pre-applied",
