...
// modules/woz_aliro/src/aliro_ble_nimble.c, shared with the ESP32 port. What is here is
// the part that names this platform, and it is smaller than the ESP-IDF equivalent
// because the port already owns NimBLE: woz_freertos_nimble_host_start() does
...
//
// The key-value store is deliberately NOT initialised here. aliro_reader_start() loads
...
#define TAG "aliro_ble"

/* Nothing is held between the two halves: aliro_ble_prepare() below copies the config
...
static const struct woz_freertos_nimble_host_hooks k_hooks = {
	.register_services = aliro_ble_register_gatt,
	.on_sync = aliro_ble_host_sync,
};
...
// begins later, from aliro_ble_host_sync() on the host task.
