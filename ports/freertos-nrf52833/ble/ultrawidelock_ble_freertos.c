...
// or a negative ultrawidelock_freertos_nimble_host_stage on failure; -1 if cfg is NULL.
//
...
int ultrawidelock_ble_start(const struct ultrawidelock_ble_config *cfg)
{
	...
	if (rc != 0) {
		ultrawidelock_freertos_log(ULTRAWIDELOCK_FREERTOS_LOG_ERROR, TAG, "host start failed at stage %d",
				 -rc);
		return rc;
	}
