 ...
 * behind IS_ENABLED(CONFIG_ALIRO_STATUS_LED), and without that macro every
 * LED call in matter_commission.c compiles to the header's no-op stub -- no
 * warning, no missing symbol, just a lamp that ignores the Door Lock cluster.
 * The port CMakeLists defines CONFIG_ALIRO_STATUS_LED=1 for exactly this
 * reason; see the note beside woz_matter_transport.
 */
#ifndef WOZ_MATTER_COMPAT_ZEPHYR_SYS_UTIL_H
#define WOZ_MATTER_COMPAT_ZEPHYR_SYS_UTIL_H
...
#endif /* WOZ_MATTER_COMPAT_ZEPHYR_SYS_UTIL_H */
