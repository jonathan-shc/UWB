 ...
 */
#ifndef ULTRAWIDELOCK_MATTER_COMPAT_ZEPHYR_KERNEL_H
#define ULTRAWIDELOCK_MATTER_COMPAT_ZEPHYR_KERNEL_H
 ...
 * void f(struct ultrawidelock_work *). Casting between the two function-pointer types
 ...
 *
 * The ultrawidelock_work member MUST stay first for that to hold.
 */
struct k_work {
...
struct k_work_delayable {
	struct ultrawidelock_dwork d;
	void (*handler)(struct k_work *work);
};

static inline void ultrawidelock_k_work_tramp(struct ultrawidelock_work *w)
{
...
static inline void ultrawidelock_k_dwork_tramp(struct ultrawidelock_dwork *d)
{
