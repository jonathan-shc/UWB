 ...
 * argument applies to woz_osal here, which the reader also uses. PASE is what
...
static void hs_work_handler(void)
{
	...
	s_hs_resp_len = 0u;
	woz_freertos_log(WOZ_FREERTOS_LOG_INFO, MATTER_BLE_TAG,
...
static void msg_work_handler(void)
{
	if (s_msg_cb != NULL) {
		...
		woz_freertos_log(WOZ_FREERTOS_LOG_WARNING, MATTER_BLE_TAG,
...
static int indicate_raw(const uint8_t *data, size_t len)
{
	...
	if (!is_subscribed()) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, MATTER_BLE_TAG,
	...
	if (om == NULL) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, MATTER_BLE_TAG, "no mbuf for indication");
		return -1;
	}
 ...
 * accepts BLE connections too, and claiming every one of them would let an Aliro
...
static bool claim_conn(uint16_t conn_handle)
{
	...
	reset_link();
	woz_freertos_log(WOZ_FREERTOS_LOG_INFO, MATTER_BLE_TAG, "commissioning link up");
	return true;
}
...
/*
 * Registered as a LISTENER, not as the connection callback. The Aliro reader
...
static int matter_gap_event(struct ble_gap_event *event, void *arg)
{
	...
	case BLE_GAP_EVENT_SUBSCRIBE:
		...
		woz_freertos_log(WOZ_FREERTOS_LOG_INFO, MATTER_BLE_TAG, "C2 indications %s",
				 s_subscribed ? "on" : "off");
...
int matter_ble_start(void)
{
	...
	return woz_freertos_nimble_host_add_hooks(&k_hooks);
}
