 ...
 * scripts/cdk-dfu.sh and scripts/woz_push.py drive a board without asking which
 ...
 * Mode" as the primary way in, because modules/woz_matter opens this window
 ...
 * woz_dfu_window_open() is public, and the Matter tranche will call it from the
...
#ifndef WOZ_DFU_WINDOW_MS
#define WOZ_DFU_WINDOW_MS 300000u
#endif
...
static int coc_arm_rx(struct ble_l2cap_chan *chan)
{
	...
	if (rx == NULL) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "out of rx buffers");
...
static void coc_pump(struct ble_l2cap_chan *chan, const uint8_t *frame, uint16_t len)
{
	...
	(void)woz_dfu_rx_frame(frame, len, rsp, &rsp_len);
	if (rsp_len == 0u) {
		return;
	}
	...
	if (sdu == NULL) {
		woz_freertos_log(WOZ_FREERTOS_LOG_WARNING, TAG, "no tx buffer for the reply");
		return;
	}
	if (os_mbuf_append(sdu, rsp, rsp_len) != 0 || ble_l2cap_send(chan, sdu) < 0) {
		...
		woz_freertos_log(WOZ_FREERTOS_LOG_WARNING, TAG, "reply not sent");
	}
}
...
static int l2cap_event_cb(struct ble_l2cap_event *event, void *arg)
{
	...
	case BLE_L2CAP_EVENT_COC_CONNECTED:
		if (event->connect.status != 0) {
			woz_freertos_log(WOZ_FREERTOS_LOG_WARNING, TAG,
					 "update channel connect status=%d",
					 event->connect.status);
			return 0;
		}
		woz_freertos_log(WOZ_FREERTOS_LOG_INFO, TAG, "update channel open");
		return 0;
	...
	case BLE_L2CAP_EVENT_COC_DISCONNECTED:
		 ...
		 * someone else's. */
		woz_dfu_rx_reset();
		woz_freertos_log(WOZ_FREERTOS_LOG_INFO, TAG, "update channel closed");
		return 0;
 ...
 * both paths hand the same bytes to woz_dfu_rx_frame() and neither knows the
 * other exists.
 *
 ...
 * it, with WOZ_DFU_ERR_CLOSED. That is a weaker gate, but the work it costs is
...
_Static_assert(WOZ_FREERTOS_SW2_GPIOTE_CHANNEL == 1u,
...
static void button_deferred(void *arg1, uint32_t arg2)
{
	(void)arg1;
	(void)arg2;
	woz_dfu_window_open(WOZ_DFU_WINDOW_MS);
}
 ...
 * woz_dfu_window_open() restarts the clock rather than counting, so the visible
...
static int button_init(void)
{
	...
	if (woz_freertos_gpiote_add_handler(button_isr) != 0) {
		return -1;
	}

	nrf_gpiote_event_configure(NRF_GPIOTE, WOZ_FREERTOS_SW2_GPIOTE_CHANNEL,
				   WOZ_FREERTOS_PIN_SW2, NRF_GPIOTE_POLARITY_HITOLO);
	nrf_gpiote_event_enable(NRF_GPIOTE, WOZ_FREERTOS_SW2_GPIOTE_CHANNEL);
 ...
 * ble_svc_gap_init() registers from inside aliro_ble_register_gatt().
 *
 * That is a difference and not a defect. Nothing on either side of this link
 * addresses a service by handle: iOS and the Aliro flow both discover by UUID,
...
static int dfu_register_services(void)
{
	...
	if (rc != 0) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "coc mempool init rc=%d", rc);
		return -1;
	}
	rc = os_mbuf_pool_init(&s_coc_mbuf_pool, &s_coc_mempool, DFU_MTU, DFU_COC_BUF_COUNT);
	if (rc != 0) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "coc mbuf pool init rc=%d", rc);
		return -1;
	}
	rc = ble_l2cap_create_server(DFU_L2CAP_PSM, DFU_MTU, l2cap_event_cb, NULL);
	if (rc != 0) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "update PSM 0x%04x rc=%d",
	...
	if (rc != 0) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "gatts_count_cfg rc=%d", rc);
		return -1;
	}
	rc = ble_gatts_add_svcs(k_gatt_svcs);
	if (rc != 0) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "gatts_add_svcs rc=%d", rc);
		return -1;
	}
	return 0;
}
