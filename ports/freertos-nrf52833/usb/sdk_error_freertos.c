 ...
 * that enumerates and then behaves arbitrarily. woz_freertos_fatal() is the
...
void app_error_handler_bare(uint32_t error_code)
{
	woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, "usb_sdk", "SDK error check failed, code %u",
			 (unsigned)error_code);
	woz_freertos_fatal("nRF5 SDK error check");
}
