 ...
 * therefore reads and writes the settings store directly (aliro_prov_*) rather
 * than the running engine's state, which in this mode does not exist. The one
 * exception is `import`, which routes through aliro_reader_import_blob so the
...
static char s_line[2u * ALIRO_PROV_BLOB_MAX + 16u];
...
static void cmd_import(const char *hex)
{
	...
	rc = aliro_reader_import_blob(s_blob, len);
	if (rc != 0) {
...
static void cmd_erase(const char *arg)
{
	...
	rc = aliro_reader_provision_clear();
	if (rc != 0) {
...
void woz_freertos_prov_console_run(void)
{
	for (;;) {
		char *arg;

		if (!woz_freertos_usb_ready()) {
		...
		while (woz_freertos_usb_ready()) {
			int n;

			(void)woz_freertos_usb_write("> ", 2);
			n = woz_freertos_usb_readline(s_line, sizeof(s_line));
			if (n < 0) {
