...
ret_code_t nrf_drv_power_usbevt_init(nrf_drv_power_usbevt_config_t const *p_config)
{
	...
	if (woz_freertos_radio_set_power_handler(usb_power_isr) != 0) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "POWER vector already claimed");
 ...
 */
#define WOZ_FREERTOS_USB_HF_SRC MPSL_CLOCK_HF_SRC_XO
...
void nrf_drv_clock_hfclk_request(nrf_drv_clock_handler_item_t *p_handler_item)
{
	...
	if (!s_clock_requested) {
		if (mpsl_clock_hfclk_src_request(WOZ_FREERTOS_USB_HF_SRC, hfclk_event) != 0) {
			woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "hfclk request refused");
			return;
		}
	 ...
	 */
	if (mpsl_clock_hfclk_src_is_running(WOZ_FREERTOS_USB_HF_SRC, &running) == 0 &&
	    running != 0) {
...
void nrf_drv_clock_hfclk_release(void)
{
	 ...
	 */
	(void)mpsl_clock_hfclk_src_release(WOZ_FREERTOS_USB_HF_SRC);
}
...
bool nrf_drv_clock_init_check(void)
{
	 ...
	 */
	return woz_freertos_radio_ready();
}
