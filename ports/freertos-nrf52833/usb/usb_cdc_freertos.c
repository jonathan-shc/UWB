...
#ifndef WOZ_FREERTOS_USB_TASK_STACK_BYTES
/*
 * Sized for the console rather than for USB. `import` runs
 * aliro_reader_import_blob, whose commit path does a software P-256 derive --
 * the heaviest single crypto step the reader has -- and it runs on this stack.
 */
#define WOZ_FREERTOS_USB_TASK_STACK_BYTES 4096u
#endif

#ifndef WOZ_FREERTOS_USB_TASK_PRIORITY
#define WOZ_FREERTOS_USB_TASK_PRIORITY (tskIDLE_PRIORITY + 1u)
#endif

#define USB_STACK_WORDS                                                                            \
	((WOZ_FREERTOS_USB_TASK_STACK_BYTES + sizeof(StackType_t) - 1u) / sizeof(StackType_t))
...
int woz_freertos_usb_start(void)
{
	...
	if (rc != NRF_SUCCESS) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "usbd init rc=%u", (unsigned)rc);
		return -1;
	}

	rc = app_usbd_class_append(app_usbd_cdc_acm_class_inst_get(&m_cdc_acm));
	if (rc != NRF_SUCCESS) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "cdc append rc=%u", (unsigned)rc);
		return -1;
	}

	s_task = xTaskCreateStatic(usb_task, "usb", (uint32_t)USB_STACK_WORDS, NULL,
				   WOZ_FREERTOS_USB_TASK_PRIORITY, s_stack, &s_task_storage);
	if (s_task == NULL) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "no task");
		return -1;
	}
	...
	if (rc != NRF_SUCCESS) {
		woz_freertos_log(WOZ_FREERTOS_LOG_ERROR, TAG, "power events rc=%u", (unsigned)rc);
		return -1;
	}

	woz_freertos_log(WOZ_FREERTOS_LOG_INFO, TAG, "USB CDC ACM up; open the serial port");
	return 0;
}

bool woz_freertos_usb_ready(void)
{
	return s_port_open;
}

int woz_freertos_usb_write(const char *data, size_t len)
{
	if (data == NULL || len == 0u) {
		return 0;
	}
...
int woz_freertos_usb_print(const char *line)
{
	static const char crlf[2] = {'\r', '\n'};

	if (line != NULL && line[0] != '\0' && woz_freertos_usb_write(line, strlen(line)) != 0) {
		return -1;
	}
	return woz_freertos_usb_write(crlf, sizeof(crlf));
}

int woz_freertos_usb_readline(char *out, size_t max)
{
	size_t len = 0;
	...
	for (;;) {
		...
		if (c == '\b' || c == 0x7f) {
			if (len > 0u) {
				...
				(void)woz_freertos_usb_write("\b \b", 3);
			}
			continue;
		}
		if (c < 0x20 || c > 0x7e) {
		...
		out[len++] = c;
		(void)woz_freertos_usb_write(&c, 1);
	}
}
