 ...
 * WOZ_DW3000_IRQ_PRIORITY, and everything else on this vector inherits it.
 ...
 */
#define WOZ_FREERTOS_GPIOTE_HANDLERS 2u

static woz_freertos_gpiote_handler s_handlers[WOZ_FREERTOS_GPIOTE_HANDLERS];
...
int woz_freertos_gpiote_add_handler(woz_freertos_gpiote_handler fn)
{
	...
	for (size_t i = 0; i < WOZ_FREERTOS_GPIOTE_HANDLERS; i++) {
		if (s_handlers[i] == fn) {
