/** @file ultrawidelock_logfmt.c — PRETTY-gated high-res timestamp + compact colored log line. */
...
static log_timestamp_t ultrawidelock_timestamp_get(void)
{
...
static void ultrawidelock_wrap_sample(struct k_timer *t)
{
	ARG_UNUSED(t);
	(void)ultrawidelock_timestamp_get();
}
static K_TIMER_DEFINE(g_wrap_timer, ultrawidelock_wrap_sample, NULL);

/* --- Custom line formatter ------------------------------------------------- */
/** @brief Small bounded sink so cbpprintf can render into a stack buffer. */
struct ultrawidelock_sink {
	char *p;
 ...
 * @param ctx Pointer to ultrawidelock_sink context.
 * @return Character appended.
 */
static int ultrawidelock_sink_out(int c, void *ctx)
{
	struct ultrawidelock_sink *s = ctx;
 ...
 */
static void ultrawidelock_sink_str(struct ultrawidelock_sink *s, const char *str)
{
...
static void ultrawidelock_msg_format(const struct log_output *output, struct log_msg *msg,
				     uint32_t flags)
{
	size_t dlen = 0;
	...
	char line[320];
	struct ultrawidelock_sink s = {line, sizeof(line)};
	char pre[48];

	(void)snprintk(pre, sizeof(pre), "%llu.%09u ", (unsigned long long)sec, ns);
	if (color) {
		ultrawidelock_sink_str(&s, A_DIM);
	}
	ultrawidelock_sink_str(&s, pre);
	if (color) {
		ultrawidelock_sink_str(&s, A_RST);
	}
	...
	if (name != NULL) {
		if (color) {
			ultrawidelock_sink_str(&s, tagc);
		}
		ultrawidelock_sink_str(&s, name);
		if (color) {
			ultrawidelock_sink_str(&s, A_RST);
		}
		ultrawidelock_sink_str(&s, " ");
	} else if (color) {
		ultrawidelock_sink_str(&s, A_BANNER);
	}
	...
	if (banner) {
		...
		char body[256];
		struct ultrawidelock_sink bs = {body, sizeof(body)};
		cbpprintf(ultrawidelock_sink_out, &bs, package);
		...
		cbpprintf(ultrawidelock_sink_out, &s, package);
	}
 ...
 * @return 0 on success.
 */
static int ultrawidelock_logfmt_init(void)
{
	...
	(void)log_set_timestamp_func(ultrawidelock_timestamp_get, g_freq);
	log_custom_output_msg_set(ultrawidelock_msg_format);
...
SYS_INIT(ultrawidelock_logfmt_init, APPLICATION, CONFIG_APPLICATION_INIT_PRIORITY);
