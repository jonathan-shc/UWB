...
/*
 * ESP32-only backend behind the shared ultrawidelock_ble_central.h. The decoding it
 ...
 * version (WRITE). Byte order mirrors ultrawidelock_ble.c: NimBLE stores 128-bit UUIDs
...
static void coc_connect(void)
{
	...
	int rc = ble_l2cap_connect(s_peer.conn_handle, s_peer.peer.spsm, ULTRAWIDELOCK_L2CAP_MTU, sdu_rx,
				   l2cap_event_cb, NULL);
			...
			struct ble_gatt_attr *attr, void *arg)
{
	...
	if (ultrawidelock_ble_central_parse_read_payload(buf, len, &s_peer.peer) != 0) {
	 ...
	 * (ultrawidelock_service.cpp:256), so the only symptom is a refused CoC several
	 ...
	 * reject anything under 3 bytes (ultrawidelock_ble.c:315, gatt_server.cpp:85). We
...
static bool advert_is_our_reader(const struct ble_hs_adv_fields *fields)
{
	...
	if (fields->svc_data_uuid16 == NULL ||
	    fields->svc_data_uuid16_len != ULTRAWIDELOCK_BLE_CENTRAL_SVC_DATA_LEN) {
		return false;
	}
	if (ultrawidelock_ble_central_parse_adv(fields->svc_data_uuid16, fields->svc_data_uuid16_len,
					&adv) != 0) {
		return false;
	}
	...
	return ultrawidelock_ble_central_adv_matches(&adv, s_cfg.reader_id) == 1;
}
...
static int coc_pools_init(void)
{
	int rc = os_mempool_init(&s_coc_mempool, ULTRAWIDELOCK_COC_BUF_COUNT,
				 ULTRAWIDELOCK_L2CAP_MTU, s_coc_mem, "ultrawidelock_coc_c");
