...
// FreeRTOS port. What stays here is the part that names ESP-IDF: NVS for NimBLE's key store,
...
static const char *TAG = "ultrawidelock_ble_esp";
...
int ultrawidelock_ble_start(const struct ultrawidelock_ble_config *cfg)
{
	...
	ble_hs_cfg.sync_cb = ultrawidelock_ble_host_sync;
	ble_hs_cfg.reset_cb = ultrawidelock_ble_host_reset;

	if (ultrawidelock_ble_register_gatt() != 0) {
		ESP_LOGE(TAG, "ultrawidelock_ble_register_gatt failed");
		return -1;
	}
