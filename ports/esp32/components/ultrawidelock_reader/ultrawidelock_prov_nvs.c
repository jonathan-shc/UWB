...
// nvs_flash_init.
/*
 * ultrawidelock_prov (NVS backend) — target-only load/store of the provisioning blob.
 * The portable serialisation + dev fallback + trust logic is in ultrawidelock_prov.c;
...
// On any failure to init NVS, open the namespace, read the blob, or deserialize it, falls back to
// the default DEV identity (via ultrawidelock_prov_dev_default) and returns a nonzero status: 1 if
// no provisioning was ever stored (namespace or key not found), -1 on any other NVS or
// deserialization error. Returns 0 on a successful load of previously stored provisioning.
		    ...
		    struct ultrawidelock_trust_store *ts)
{
	...
	nvs_handle_t h;
	esp_err_t err = nvs_open(ULTRAWIDELOCK_PROV_NS, NVS_READONLY, &h);
	...
	err = nvs_get_blob(h, ULTRAWIDELOCK_PROV_KEY, buf, &sz);
	nvs_close(h);
			     ...
			     const struct ultrawidelock_trust_store *ts)
{
	...
	nvs_handle_t h;
	esp_err_t err = nvs_open(ULTRAWIDELOCK_PROV_NS, NVS_READWRITE, &h);

	if (err != ESP_OK) {
		ESP_LOGW(TAG, "nvs_open(rw) err=0x%x", err);
		return -1;
	}
	err = nvs_set_blob(h, ULTRAWIDELOCK_PROV_KEY, buf, n);
	if (err == ESP_OK) {
		err = nvs_commit(h);
	}
	nvs_close(h);
