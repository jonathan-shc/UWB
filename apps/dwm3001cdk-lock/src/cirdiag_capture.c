 ...
 *
 * With CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_WINDOWS the windowed-CIR dump is armed --
...
static void cirdiag_capture_thread(void *a, void *b, void *c)
{
...
#if defined(CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_WINDOWS)
	LOG_WRN("CIR capture image: %d ms armed, %d ms idle. NOT a shipping build.",
		CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_WINDOW_MS, CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_IDLE_MS);

	for (uint32_t n = 1;; n++) {
		ultrawidelock_printf("cir.cycle: n=%u armed\n", (unsigned)n);
		uwb_cirdiag_dump_set_enabled(true);

		k_msleep(CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_WINDOW_MS);
		...
		k_msleep(CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_IDLE_MS);
	}
#else
	...
	LOG_WRN("CIR capture image: SUMMARY ONLY, %d ms per interval. NOT a shipping build.",
		CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_WINDOW_MS);
...
K_THREAD_DEFINE(cirdiag_capture_tid, CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_STACK, cirdiag_capture_thread,
		NULL, NULL, NULL, CONFIG_ULTRAWIDELOCK_CIRDIAG_CAPTURE_PRIO, 0, 0);
