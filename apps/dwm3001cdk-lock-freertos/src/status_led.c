...
static const uint32_t s_led_pin[LED_COUNT] = {
	[LED_LOCK] = ULTRAWIDELOCK_FREERTOS_PIN_LED_LOCK,
	[LED_ATTN] = ULTRAWIDELOCK_FREERTOS_PIN_LED_ATTN,
	[LED_RADIO] = ULTRAWIDELOCK_FREERTOS_PIN_LED_RADIO,
	[LED_WINDOW] = ULTRAWIDELOCK_FREERTOS_PIN_LED_WINDOW,
};
 ...
 */
#ifndef ULTRAWIDELOCK_FREERTOS_STATUS_LED_HEARTBEAT
#define ULTRAWIDELOCK_FREERTOS_STATUS_LED_HEARTBEAT 1
#endif
#if ULTRAWIDELOCK_FREERTOS_STATUS_LED_HEARTBEAT
...
static struct ultrawidelock_dwork s_tick;
 ...
 */
static void tick(struct ultrawidelock_dwork *dwork)
{
	...
	if (moving) {
		(void)ultrawidelock_dwork_reschedule(&s_tick, SLOT_MS);
	}
}
...
void status_led_signal(uint32_t signals, bool on)
{
	 ...
	 */
	s_phase = 0u;
	(void)ultrawidelock_dwork_reschedule(&s_tick, 0);
}
...
void status_led_boot_blink(void)
{
	 ...
	 *
	 * Unlike the Zephyr side there is no cancel-and-wait: ultrawidelock_dwork_cancel
	 ...
	 * reads back.
	 */
	(void)ultrawidelock_dwork_cancel(&s_tick);
...
void status_led_start(void)
{
	...
	ultrawidelock_dwork_init(&s_tick, tick);
	(void)ultrawidelock_dwork_reschedule(&s_tick, 0);
}
