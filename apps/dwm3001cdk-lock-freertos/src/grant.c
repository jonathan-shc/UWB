...
void grant_init(struct grant_ctx *ctx, uint32_t gen_now)
{
	 ...
	 */
	aliro_approach_init(&ctx->approach, NULL);
...
void grant_step(struct grant_ctx *ctx, const struct grant_input *in, struct grant_output *out)
{
	...
	if (in->gen != ctx->gen_trusted && in->trusted_valid) {
		...
		ctx->present = true;
		act = aliro_approach_feed(&ctx->approach, in->now_ms, in->trusted_cm);
	} else {
		/*
		...
		if (in->gen != ctx->gen_observed) {
			ctx->gen_observed = in->gen;
			if (in->raw_valid) {
				aliro_approach_observe_departure(&ctx->approach, in->now_ms,
								 in->raw_cm);
			}
		}
		act = aliro_approach_tick(&ctx->approach, in->now_ms);
	}
	...
	case ALIRO_APPROACH_UNLOCK_THRESHOLD:
		 ...
		 * CONFIG_WOZ_SIDE_GATE, which is the shipped default.
		 */
		if (!ctx->granted) {
	...
	if (in->session_active && !ctx->session_was_up) {
		aliro_approach_session_up(&ctx->approach);
	}
	ctx->session_was_up = in->session_active;

	/*
	 * Departure: the peer's Aliro session ended. Ranging silence alone does
