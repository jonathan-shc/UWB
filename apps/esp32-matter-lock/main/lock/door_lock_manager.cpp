// BoltLockManager: Matter door lock cluster backing store for the ESP32 port. Implements the
// DoorLock cluster's user, credential, and weekday/yearday/holiday schedule get/set callbacks over
// fixed-size in-memory tables mirrored to NVM (ESP32Config blobs), plus lock/unlock actuation and
// PIN validation. Cluster indices are one-indexed by Matter and decremented internally before
// bounds-checking against this platform's fixed capacity limits.
/*
...
bool ReadOptionalConfigBlob(ESP32Config::Key key, uint8_t *data, size_t dataLen, bool *outStale)
{
    ...
    if (outLen != dataLen) {
	    ESP_LOGW(TAG,
		     "Door lock config blob '%s' has stale size %u, expected %u; reinitialising",
		     key.Name, static_cast<unsigned>(outLen), static_cast<unsigned>(dataLen));
	    *outStale = true;
	    return true;
    }
    return true;
...
// them. Returns CHIP_ERROR_NO_MEMORY if any configured count exceeds its platform maximum,
// otherwise CHIP_NO_ERROR. The state parameter is accepted but unused.
                                 ...
                                 LockParam lockParam)
{
    ...
    if (LockParams.numberOfUsers > kMaxUsers) {
	    ESP_LOGI(TAG,
		     "Max number of users is greater than %d, the maximum amount of users "
    ...
    if (LockParams.numberOfCredentialsPerUser > kMaxCredentialsPerUser) {
	    ESP_LOGI(TAG,
		     "Max number of credentials per user is greater than %d, the maximum amount of "
		     "users currently supported on "
		     "this platform",
    ...
    if (LockParams.numberOfWeekdaySchedulesPerUser > kMaxWeekdaySchedulesPerUser) {
	    ESP_LOGI(TAG,
		     " Max number of schedules is greater than %d, the maximum amount of schedules "
		     "currently supported on this platform",
...
uint16_t BoltLockManager::CredentialStorageIndex(uint16_t credentialIndex,
						 CredentialTypeEnum type) const
{
...
// Loads all persisted door-lock state (users, credentials, names, credential data, per-user
// credential links, weekday/yearday/holiday schedules) from NVM into the in-memory tables. Detects
// a stale database, either by a blob whose stored size does not match this build's table size or by
// credential slot 0 holding a non-programming-PIN type (an old mixed-type layout); in either case
// clears every in-memory table and rewrites all blobs to NVM. Returns true if all blob reads (and,
// if triggered, all stale-format rewrites) succeed; false if any read or rewrite fails.
//
...
bool BoltLockManager::ReadConfigValues()
{
    bool ok = true;
    bool stale = false;
    ok &= ReadOptionalConfigBlob(ESP32Config::kConfigKey_LockUser,
				 reinterpret_cast<uint8_t *>(mLockUsers), sizeof(mLockUsers),
				 &stale);
    ok &= ReadOptionalConfigBlob(ESP32Config::kConfigKey_Credential,
				 reinterpret_cast<uint8_t *>(mLockCredentials),
				 sizeof(mLockCredentials), &stale);
    ok &= ReadOptionalConfigBlob(ESP32Config::kConfigKey_LockUserName,
				 reinterpret_cast<uint8_t *>(mUserNames), sizeof(mUserNames),
				 &stale);
    ok &= ReadOptionalConfigBlob(ESP32Config::kConfigKey_CredentialData,
				 reinterpret_cast<uint8_t *>(mCredentialData),
				 sizeof(mCredentialData), &stale);
    ok &= ReadOptionalConfigBlob(ESP32Config::kConfigKey_UserCredentials,
				 reinterpret_cast<uint8_t *>(mCredentials), sizeof(mCredentials),
				 &stale);
    ok &= ReadOptionalConfigBlob(ESP32Config::kConfigKey_WeekDaySchedules,
				 reinterpret_cast<uint8_t *>(mWeekdaySchedule),
				 sizeof(mWeekdaySchedule), &stale);
    ok &= ReadOptionalConfigBlob(ESP32Config::kConfigKey_YearDaySchedules,
				 reinterpret_cast<uint8_t *>(mYeardaySchedule),
				 sizeof(mYeardaySchedule), &stale);
    ok &= ReadOptionalConfigBlob(ESP32Config::kConfigKey_HolidaySchedules,
	...
	ok &= WriteConfigBlob(ESP32Config::kConfigKey_LockUser,
			      reinterpret_cast<const uint8_t *>(&mLockUsers), sizeof(mLockUsers));
	ok &= WriteConfigBlob(ESP32Config::kConfigKey_Credential,
			      reinterpret_cast<const uint8_t *>(&mLockCredentials),
			      sizeof(mLockCredentials));
	ok &= WriteConfigBlob(ESP32Config::kConfigKey_LockUserName,
			      reinterpret_cast<const uint8_t *>(mUserNames), sizeof(mUserNames));
	ok &= WriteConfigBlob(ESP32Config::kConfigKey_CredentialData,
			      reinterpret_cast<const uint8_t *>(&mCredentialData),
			      sizeof(mCredentialData));
	ok &= WriteConfigBlob(ESP32Config::kConfigKey_UserCredentials,
			      reinterpret_cast<const uint8_t *>(mCredentials),
			      sizeof(mCredentials));
	ok &= WriteConfigBlob(ESP32Config::kConfigKey_WeekDaySchedules,
			      reinterpret_cast<const uint8_t *>(mWeekdaySchedule),
			      sizeof(mWeekdaySchedule));
	ok &= WriteConfigBlob(ESP32Config::kConfigKey_YearDaySchedules,
			      reinterpret_cast<const uint8_t *>(mYeardaySchedule),
			      sizeof(mYeardaySchedule));
	ok &= WriteConfigBlob(ESP32Config::kConfigKey_HolidaySchedules,
...
// LED. Drives the LED's credential-specific indication when source is kAliro.
// userIndex: one-indexed user the operation is attributed to in the LockOperation event, or null
// when the operation cannot be traced to a stored user.
...
uint16_t BoltLockManager::UserIndexForAliroCredential(const ByteSpan  &credentialData)
{
    for (uint16_t userIndex = 0; userIndex < kMaxUsers; userIndex++) {
	    ...
	    if (link.credentialIndex == 0 ||
		!IsValidCredentialIndex(link.credentialIndex - 1, link.credentialType)) {
		    continue;
	    }
	    uint16_t storageIndex =
		    CredentialStorageIndex(link.credentialIndex - 1, link.credentialType);
	    if (DlCredentialStatus::kOccupied != mLockCredentials[storageIndex].status) {
                continue;
            }
	    if (credentialData.data_equal(
			ByteSpan(mCredentialData[storageIndex], kMaxCredentialSize))) {
...
// user: output; on an available (unoccupied) slot only userStatus is set; on an occupied slot name,
// credentials, unique ID, type, credential rule, and creator/modifier are also filled in, with
// creation/modification source always reported as Matter. Returns true if userIndex is valid,
// whether or not the slot is occupied; returns false only if userIndex (0 or out of range) is
// invalid.
bool BoltLockManager::GetUser(EndpointId endpointId, uint16_t userIndex,
			      EmberAfPluginDoorLockUserInfo &user)
{
    ...
    ESP_LOGI(TAG, "Door Lock App: BoltLockManager::GetUser [endpoint=%d,userIndex=%hu]", endpointId,
	     userIndex);
    ...
    user.credentials =
	    Span<const CredentialStruct>(mCredentials[userIndex], userInDb.credentials.size());
    user.userUniqueId   = userInDb.userUniqueId;
    user.userType       = userInDb.userType;
    user.credentialRule = userInDb.credentialRule;
    // So far there's no way to actually create the credential outside Matter, so here we always set
    // the creation/modification source to Matter
    ...
    ESP_LOGI(TAG,
	     ...
	     endpointId, static_cast<int>(user.userName.size()), user.userName.data(),
	     user.credentials.size(), user.userUniqueId, to_underlying(user.userType),
...
// links to NVM. userIndex: one-indexed by the caller; decremented here and validated against
// kMaxUsers. creator, modifier: fabric indices recorded as the user's creator and last modifier.
// userName: must not exceed DOOR_LOCK_MAX_USER_NAME_SIZE.
// credentials, totalCredentials: credential list to associate with the user; totalCredentials must
// not exceed LockParams.numberOfCredentialsPerUser. Returns false if userIndex is invalid, userName
// is too long, totalCredentials is too large, or the NVM write fails; true on success.
			      ...
			      const CredentialStruct *credentials, size_t totalCredentials)
{
	ESP_LOGI(TAG,
		 ...
		 endpointId, userIndex, creator, modifier, userName.data(), uniqueId,
		 to_underlying(userStatus), to_underlying(usertype), to_underlying(credentialRule),
	...
	if (userName.size() > DOOR_LOCK_MAX_USER_NAME_SIZE) {
		ESP_LOGE(TAG, "Cannot set user - user name is too long [endpoint=%d,index=%d]",
    ...
    if (totalCredentials > LockParams.numberOfCredentialsPerUser) {
	    ESP_LOGE(TAG,
		     "Cannot set user - total number of credentials is too big "
		     "[endpoint=%d,index=%d,totalCredentials=%u]",
    ...
    userInStorage.credentials =
	    Span<const CredentialStruct>(mCredentials[userIndex], totalCredentials);

    // Save user information in NVM flash
    if (!WriteConfigBlob(ESP32Config::kConfigKey_LockUser,
			 reinterpret_cast<const uint8_t *>(mLockUsers), sizeof(mLockUsers)) ||
	!WriteConfigBlob(ESP32Config::kConfigKey_UserCredentials,
			 reinterpret_cast<const uint8_t *>(mCredentials), sizeof(mCredentials)) ||
	!WriteConfigBlob(ESP32Config::kConfigKey_LockUserName,
			 reinterpret_cast<const uint8_t *>(mUserNames), sizeof(mUserNames))) {
	    return false;
    }
...
// Programming-PIN indices are used as-is; all other credential types are one-indexed by the caller
// and decremented here. endpointId: unused, present for cluster callback signature compatibility.
// credentialIndex: caller-supplied index; must pass IsValidCredentialIndex after the type-specific
// decrement. credentialType: type of credential being looked up. credential: output; on an
// available (unoccupied) slot only status is set; on an occupied slot type, data, and
// creator/modifier are also filled in, with creation/modification source always reported as Matter.
// Returns true if credentialIndex is valid, whether or not the slot is occupied; returns false only
// if credentialIndex is invalid for the given type.
bool BoltLockManager::GetCredential(EndpointId endpointId, uint16_t credentialIndex,
				    ...
				    EmberAfPluginDoorLockCredentialInfo &credential)
{
    if (CredentialTypeEnum::kProgrammingPIN == credentialType) {
	    ...
	    VerifyOrReturnValue(IsValidCredentialIndex(--credentialIndex, credentialType),
				false); // otherwise, indices are one-indexed
    }

    ESP_LOGI(TAG,
	     "Lock App: BoltLockManager::GetCredential [credentialType=%u], credentialIndex=%d",
    ...
    ESP_LOGI(TAG, "CredentialStatus: %d, CredentialIndex: %d, StorageIndex: %d ",
	     (int)credential.status, credentialIndex, storageIndex);
    ...
    ESP_LOGI(TAG, "Found occupied credential [type=%u,dataSize=%u]",
...
// and decremented here. endpointId: unused, present for cluster callback signature compatibility.
// credentialIndex: caller-supplied index; must pass IsValidCredentialIndex after the type-specific
// decrement. creator, modifier: fabric indices recorded as the credential's creator and last
// modifier. credentialStatus: status to store for this credential slot. credentialType: type of
// credential being stored. credentialData: raw credential bytes; must not exceed
// kMaxCredentialSize. Returns true on success; false if credentialIndex is invalid, credentialData
// exceeds kMaxCredentialSize, or the NVM write fails.
bool BoltLockManager::SetCredential(EndpointId endpointId, uint16_t credentialIndex,
				    ...
				    const ByteSpan &credentialData)
{
    ...
    credentialInStorage.credentialData =
	    ByteSpan{mCredentialData[storageIndex], credentialData.size()};
    // Save credential information in NVM flash
    if (!WriteConfigBlob(ESP32Config::kConfigKey_Credential,
			 reinterpret_cast<const uint8_t *>(&mLockCredentials),
			 sizeof(mLockCredentials)) ||
	!WriteConfigBlob(ESP32Config::kConfigKey_CredentialData,
			 reinterpret_cast<const uint8_t *>(&mCredentialData),
			 sizeof(mCredentialData))) {
	    return false;
    }
    ESP_LOGI(TAG, "Successfully set the credential [credentialType=%u]",
	     to_underlying(credentialType));

    return true;
}

// Looks up a stored weekday schedule by weekday index and user index for the Matter door lock
// cluster. weekdayIndex, userIndex: both one-indexed by the caller; decremented here and validated
// against their respective max counts. schedule: output, populated only when the slot is occupied.
// Returns DlStatus::kFailure if either index is 0 or out of range, kNotFound if the slot is
// unoccupied, kSuccess otherwise.
DlStatus BoltLockManager::GetWeekdaySchedule(EndpointId endpointId, uint8_t weekdayIndex,
					     uint16_t userIndex,
...
// Stores a weekday schedule by weekday index and user index for the Matter door lock cluster,
// persisting it to NVM. weekdayIndex, userIndex: both one-indexed by the caller; decremented here
// and validated against their respective max counts. status, daysMask, startHour, startMinute,
// endHour, endMinute: schedule fields written to the slot. Returns DlStatus::kFailure if either
// index is 0, out of range, or the NVM write fails; kSuccess otherwise.
DlStatus BoltLockManager::SetWeekdaySchedule(EndpointId endpointId, uint8_t weekdayIndex,
					     ...
					     uint8_t endMinute)
{
    ...
    if (!WriteConfigBlob(ESP32Config::kConfigKey_WeekDaySchedules,
			 reinterpret_cast<const uint8_t *>(mWeekdaySchedule),
			 sizeof(mWeekdaySchedule))) {
...
// Looks up a stored yearday schedule by yearday index and user index for the Matter door lock
// cluster. yearDayIndex, userIndex: both one-indexed by the caller; decremented here and validated
// against their respective max counts. schedule: output, populated only when the slot is occupied.
// Returns DlStatus::kFailure if either index is 0 or out of range, kNotFound if the slot is
// unoccupied, kSuccess otherwise.
DlStatus BoltLockManager::GetYeardaySchedule(EndpointId endpointId, uint8_t yearDayIndex,
					     uint16_t userIndex,
...
// Stores a yearday schedule by yearday index and user index for the Matter door lock cluster,
// persisting it to NVM. yearDayIndex, userIndex: both one-indexed by the caller; decremented here
// and validated against their respective max counts. status, localStartTime, localEndTime: schedule
// fields written to the slot. Returns DlStatus::kFailure if either index is 0, out of range, or the
// NVM write fails; kSuccess otherwise.
DlStatus BoltLockManager::SetYeardaySchedule(EndpointId endpointId, uint8_t yearDayIndex,
					     uint16_t userIndex, DlScheduleStatus status,
					     uint32_t localStartTime, uint32_t localEndTime)
{
    ...
    // Save schedule information in NVM flash
    if (!WriteConfigBlob(ESP32Config::kConfigKey_YearDaySchedules,
			 reinterpret_cast<const uint8_t *>(mYeardaySchedule),
			 sizeof(mYeardaySchedule))) {
	    return DlStatus::kFailure;
    }
...
// holidayIndex: one-indexed by the caller; decremented here and validated against
// kMaxHolidaySchedules. schedule: output, populated only when the slot is occupied. Returns
// DlStatus::kFailure if holidayIndex is 0 or out of range, kNotFound if the slot is unoccupied,
// kSuccess otherwise.
...
// holidayIndex: one-indexed by the caller; decremented here and validated against
// kMaxHolidaySchedules. status, localStartTime, localEndTime, operatingMode: schedule fields
// written to the slot. Returns DlStatus::kFailure if holidayIndex is 0, out of range, or the NVM
// write fails; kSuccess otherwise.
DlStatus BoltLockManager::SetHolidaySchedule(EndpointId endpointId, uint8_t holidayIndex,
					     ...
					     uint32_t localEndTime, OperatingModeEnum operatingMode)
{
    ...
    // Save schedule information in NVM flash
    if (!WriteConfigBlob(ESP32Config::kConfigKey_HolidaySchedules,
			 reinterpret_cast<const uint8_t *>(mHolidaySchedule),
			 sizeof(mHolidaySchedule))) {
	    return DlStatus::kFailure;
    }
...
// If no PIN is supplied, succeeds unless the RequirePINforRemoteOperation attribute is true
// (defaults to not required if the attribute read fails). If a PIN is supplied, succeeds if it
// exact-matches any occupied PIN credential slot. endpointId: endpoint whose
// RequirePINforRemoteOperation attribute is checked. pin: PIN to validate, or empty to check
// whether a PIN is required. err: output; set to OperationErrorEnum::kInvalidCredential when a
// supplied PIN matches no stored credential. Returns true if no PIN was required or a match was
// found; false otherwise.
bool BoltLockManager::ValidatePIN(EndpointId endpointId, const Optional<ByteSpan> &pin,
				  OperationErrorEnum &err) const
{
	...
	ESP_LOGI(TAG,
		 "Door Lock App: PIN code is not specified, but it is required [endpointId=%d]",
		 endpointId);
	return false;
    }

    // Check the PIN code
    for (uint16_t credentialIndex = 0; credentialIndex < kMaxCredentialsPerUser;
	 ++credentialIndex) {
	    const uint16_t storageIndex =
		    CredentialStorageIndex(credentialIndex, CredentialTypeEnum::kPin);
	    const auto &credentialInStorage = mLockCredentials[storageIndex];
	    ...
	    if (credentialInStorage.credentialData.data_equal(pin.Value())) {
		    ESP_LOGI(TAG,
			     "Lock App: specified PIN code was found in the database "
			     "[endpointId=%d]",
			     endpointId);
		    return true;
	    }
    ...
    ESP_LOGI(TAG, "Door Lock App: specified PIN code was not found in the database [endpointId=%d]",
	     endpointId);
    err = OperationErrorEnum::kInvalidCredential;
    return false;
}

// Initializes the door lock's runtime state: reads the initial lock state and per-endpoint
// user/credential capacity attributes (falling back to 5 credentials-per-user and 10 users on read
// failure), initializes BoltLockMgr with those limits, then loads persisted state from NVM. Returns
// CHIP_NO_ERROR on success; the error from BoltLockMgr().Init() if lock-parameter validation fails;
// CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND if loading persisted config fails.
CHIP_ERROR BoltLockManager::InitLockState()
{
    ...
    if (!DoorLockServer::Instance().GetNumberOfCredentialsSupportedPerUser(
		endpointId, numberOfCredentialsPerUser)) {
	    ESP_LOGE(TAG,
		     "Unable to get number of credentials supported per user when initializing "
		     "lock endpoint, defaulting to 5 [endpointId=%d]",
		     endpointId);
    ...
    if (!DoorLockServer::Instance().GetNumberOfUserSupported(endpointId, numberOfUsers)) {
	    ESP_LOGE(TAG,
		     "Unable to get number of supported users when initializing lock endpoint, "
		     "defaulting to 10 [endpointId=%d]",
		     endpointId);
	    numberOfUsers = 10;
    }
