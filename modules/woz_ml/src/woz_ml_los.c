...
#include "woz_ml.h"
#include "woz_ml_los_scaler.h"
#include "woz_ml_los_tree.h"
...
{
	int16_t q[WOZ_ML_LOS_N_FEATURES];

	for (int i = 0; i < WOZ_ML_LOS_N_FEATURES; i++) {
		q[i] = quantise(feat[i], woz_ml_los_lo[i], woz_ml_los_scale[i]);
	}

	/* The generated predictor returns the class index directly. It cannot
	 * fail: every path through the tree ends in a leaf. */
	return (enum woz_ml_los_class)woz_ml_los_tree_predict(q, WOZ_ML_LOS_N_FEATURES);
}
...
{
	return (pwr_diff_db > VENDOR_THRESHOLD_DB) ? WOZ_ML_LOS_OBSTRUCTED : WOZ_ML_LOS_CLEAR;
}
...
bool woz_ml_los_disagrees(const float feat[WOZ_ML_LOS_N_FEATURES], float pwr_diff_db)
{
	...
	return woz_ml_los_classify(feat) != woz_ml_los_vendor(pwr_diff_db);
}
