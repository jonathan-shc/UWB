 ...
 * explain, the classifier's strongest feature. log10f would cost woz_ml its
 * no-undefined-symbols property, so woz_ml_log2.c supplies the logarithm and
...
float woz_ml_los_range_correction(uint16_t dist_cm)
{
	...
	return K_DB_PER_LOG2 * (woz_ml_log2_u64(d) - LOG2_REF_CM);
}

float woz_ml_los_fp_resid(float fp_pwr_db, uint16_t dist_cm)
{
	return fp_pwr_db + woz_ml_los_range_correction(dist_cm);
}
