				...
				struct ultrawidelock_cred_ble_message *message, size_t *consumed)
{
	...
	if (data_length < ULTRAWIDELOCK_CRED_BLE_HEADER_SIZE) {
		return ULTRAWIDELOCK_CRED_BLE_TRUNCATED;
	}
	if ((data[0] & 0xc0u) != 0) {
	...
	if (payload_length > data_length - ULTRAWIDELOCK_CRED_BLE_HEADER_SIZE) {
		return ULTRAWIDELOCK_CRED_BLE_TRUNCATED;
	}
	message->protocol = data[0];
	message->message_id = data[1];
	message->payload = data + ULTRAWIDELOCK_CRED_BLE_HEADER_SIZE;
	message->payload_length = payload_length;
	*consumed = ULTRAWIDELOCK_CRED_BLE_HEADER_SIZE + payload_length;
					 ...
					 size_t *output_length)
{
	...
	if (output_capacity < ULTRAWIDELOCK_CRED_BLE_HEADER_SIZE ||
	    payload_length > output_capacity - ULTRAWIDELOCK_CRED_BLE_HEADER_SIZE) {
		return ULTRAWIDELOCK_CRED_BLE_BUFFER_TOO_SMALL;
	}
	output[0] = protocol;
	output[1] = message_id;
	output[2] = (uint8_t)(payload_length >> 8);
	output[3] = (uint8_t)payload_length;
	memcpy(output + ULTRAWIDELOCK_CRED_BLE_HEADER_SIZE, payload, payload_length);
	*output_length = ULTRAWIDELOCK_CRED_BLE_HEADER_SIZE + payload_length;
	return ULTRAWIDELOCK_CRED_BLE_OK;
}
	...
	const uint8_t **proprietary_information, size_t *proprietary_information_length)
{
	...
	if (message->protocol != ULTRAWIDELOCK_CRED_BLE_PROTOCOL_NOTIFICATION ||
	...
	size_t offset = 0;
	if (ultrawidelock_cred_tlv_next(encoded, encoded_length, &offset, &tlv) !=
	...
	const struct ultrawidelock_cred_ble_message *message)
{
	...
	return message->protocol == ULTRAWIDELOCK_CRED_BLE_PROTOCOL_UWB ||
...
static int build_status(uint8_t message_id, uint8_t first, uint8_t reader_state, uint8_t output[8])
{
	...
	output[0] = ULTRAWIDELOCK_CRED_BLE_PROTOCOL_NOTIFICATION;
	output[1] = message_id;
	output[2] = 0;
	output[3] = 4;
...
int ultrawidelock_cred_ble_build_access_completed(uint8_t reader_capabilities, uint8_t reader_state,
					 uint8_t output[8])
{
	return build_status(ULTRAWIDELOCK_CRED_BLE_NOTIFICATION_ACCESS_COMPLETED, reader_capabilities,
			    reader_state, output);
}
...
{
	return build_status(ULTRAWIDELOCK_CRED_BLE_NOTIFICATION_READER_STATUS_CHANGED, operation_source,
			    reader_state, output);
}
