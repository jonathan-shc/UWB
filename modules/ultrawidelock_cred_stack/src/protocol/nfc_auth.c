...
/**
 * Write a TLV (Tag-Length-Value) field to output buffer at *offset; return
 * ULTRAWIDELOCK_CRED_AUTH_OK on success or ULTRAWIDELOCK_CRED_AUTH_BUFFER_TOO_SMALL if capacity is
 * exceeded.
 */
static int put_tlv(uint8_t *output, size_t capacity, size_t *offset, uint32_t tag,
		   const uint8_t *value, size_t length)
{
	return ultrawidelock_cred_tlv_write(output, capacity, offset, tag, value, length) ==
 ...
 * identifiers, optional vendor extension; return ULTRAWIDELOCK_CRED_AUTH_OK on success or error
 * code if arguments are invalid or output capacity is exceeded.
 */
					   ...
					   size_t *output_length)
{
	if (params == NULL || output == NULL || output_length == NULL ||
		...
		return ULTRAWIDELOCK_CRED_AUTH_INVALID_ARGUMENT;
	}
	const size_t body_length =
		129 + (params->vendor_extension_length == 0
			       ? 0
			       : ultrawidelock_cred_tlv_encoded_size(0xb1, params->vendor_extension_length));
	if (body_length > 255 || output_capacity < body_length + 6) {
		return ULTRAWIDELOCK_CRED_AUTH_BUFFER_TOO_SMALL;
	}
	...
	*output_length = offset;
	return ULTRAWIDELOCK_CRED_AUTH_OK;
}
 ...
 * extension (tag 0xb2); return ULTRAWIDELOCK_CRED_AUTH_OK on success or error code on malformed
 * input or status error.
 */
				   ...
				   int fast_requested, struct ultrawidelock_cred_auth0_response *result)
{
	...
	if (response_length < 2) {
		return ULTRAWIDELOCK_CRED_AUTH_INVALID_APDU;
	}
	if (response[response_length - 2] != 0x90 || response[response_length - 1] != 0x00) {
		return ULTRAWIDELOCK_CRED_AUTH_STATUS_ERROR;
	}

	size_t offset = 0;
	const size_t data_length = response_length - 2;
	struct ultrawidelock_cred_tlv tlv;
	if (ultrawidelock_cred_tlv_next(response, data_length, &offset, &tlv) !=
		    ULTRAWIDELOCK_CRED_TLV_OK ||
	    tlv.tag != 0x86 || tlv.length != ULTRAWIDELOCK_CRED_PUBLIC_KEY_SIZE ||
	    tlv.value[0] != 0x04) {
		return ULTRAWIDELOCK_CRED_AUTH_WRONG_CONTENT;
	}
	memcpy(result->credential_ephemeral_public_key, tlv.value, ULTRAWIDELOCK_CRED_PUBLIC_KEY_SIZE);
	...
	if (offset < data_length) {
		...
		if (tlv.tag == 0x9d) {
			...
			if (offset < data_length &&
			    ultrawidelock_cred_tlv_next(response, data_length, &offset, &tlv) !=
				    ULTRAWIDELOCK_CRED_TLV_OK) {
				return ULTRAWIDELOCK_CRED_AUTH_INVALID_APDU;
			}
		}
	...
	uint8_t output[ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE])
{
	...
	size_t offset = 0;
	if (put_tlv(output, ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE, &offset, 0x4d, reader_identifier,
		    ULTRAWIDELOCK_CRED_READER_ID_SIZE) ||
	    put_tlv(output, ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE, &offset, 0x86,
		    credential_ephemeral_public_key + 1, 32) ||
	    put_tlv(output, ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE, &offset, 0x87,
		    reader_ephemeral_public_key + 1, 32) ||
	    put_tlv(output, ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE, &offset, 0x4c, transaction_identifier,
		    ULTRAWIDELOCK_CRED_TRANSACTION_ID_SIZE) ||
	    put_tlv(output, ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE, &offset, 0x93, usage_bytes,
		    sizeof(usage_bytes)) ||
	    offset != ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE) {
		return ULTRAWIDELOCK_CRED_AUTH_BUFFER_TOO_SMALL;
	}
	return ULTRAWIDELOCK_CRED_AUTH_OK;
}
 ...
 * ULTRAWIDELOCK_CRED_AUTH_OK on success or error code if arguments are invalid or output capacity
 * is exceeded.
 */
...
{
	return ultrawidelock_cred_build_auth1_command_ex(command_parameters, signature, NULL, 0, output,
				     ...
				     size_t output_capacity, size_t *output_length)
{
	...
	const size_t certificate_tlv_length =
		reader_certificate_length == 0
			? 0
			: ultrawidelock_cred_tlv_encoded_size(0x90, reader_certificate_length);
	...
	if (put_tlv(output, output_capacity, &offset, 0x41, &command_parameters, 1) ||
	    put_tlv(output, output_capacity, &offset, 0x9e, signature,
		    ULTRAWIDELOCK_CRED_SIGNATURE_SIZE) ||
	    (reader_certificate_length != 0 &&
	...
	output[offset++] = 0;
	*output_length = offset;
	return offset == body_length + 6 ? ULTRAWIDELOCK_CRED_AUTH_OK
					 : ULTRAWIDELOCK_CRED_AUTH_INVALID_APDU;
}
 ...
 * signed timestamps (tags 0x91, 0x92); return ULTRAWIDELOCK_CRED_AUTH_OK on success or error code
 * if format is invalid or required fields are missing.
 */
				    ...
				    struct ultrawidelock_cred_auth1_response *result)
{
	...
	size_t offset = 0;
	struct ultrawidelock_cred_tlv tlv;
	if (ultrawidelock_cred_tlv_next(plaintext, plaintext_length, &offset, &tlv) !=
	    ULTRAWIDELOCK_CRED_TLV_OK) {
		return ULTRAWIDELOCK_CRED_AUTH_INVALID_APDU;
	}
	if (!public_key_requested || tlv.tag != 0x5a || tlv.length != ULTRAWIDELOCK_CRED_PUBLIC_KEY_SIZE ||
	    tlv.value[0] != 0x04) {
		return ULTRAWIDELOCK_CRED_AUTH_WRONG_CONTENT;
	}
	memcpy(result->credential_public_key, tlv.value, ULTRAWIDELOCK_CRED_PUBLIC_KEY_SIZE);
	if (ultrawidelock_cred_tlv_next(plaintext, plaintext_length, &offset, &tlv) !=
		    ULTRAWIDELOCK_CRED_TLV_OK ||
	    tlv.tag != 0x9e || tlv.length != ULTRAWIDELOCK_CRED_SIGNATURE_SIZE) {
		return ULTRAWIDELOCK_CRED_AUTH_WRONG_CONTENT;
	}
	memcpy(result->signature, tlv.value, ULTRAWIDELOCK_CRED_SIGNATURE_SIZE);
	...
	return found_signaling ? ULTRAWIDELOCK_CRED_AUTH_OK : ULTRAWIDELOCK_CRED_AUTH_WRONG_CONTENT;
}
