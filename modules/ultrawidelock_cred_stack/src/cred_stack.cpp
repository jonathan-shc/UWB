...
LOG_MODULE_REGISTER(ultrawidelock_cred_source, CONFIG_NCS_ALIRO_LOG_LEVEL_VALUE);
...
/**
 * Copies dynamicTag (kULTRAWIDELOCK_CRED_DYNAMIC_TAG_SIZE bytes) into mDynamicTag.
 */
	...
	BleTypes::BleExpiryTimestamp expirationTime)
{
	...
	std::array<uint8_t, ULTRAWIDELOCK_CRED_DYNAMIC_TAG_INPUT_SIZE> plaintext{};
	std::array<uint8_t, ULTRAWIDELOCK_CRED_DYNAMIC_TAG_INPUT_SIZE> ciphertext{};
	std::array<uint8_t, ULTRAWIDELOCK_CRED_DYNAMIC_TAG_SIZE> dynamicTag{};
	ultrawidelock_cred_dynamic_tag_input(address.data(), expirationTime, plaintext.data());
	AliroError error =
		Interface::Crypto::Encrypt(plaintext.data(), plaintext.size(), ciphertext.data());
	if (error != ALIRO_NO_ERROR) {
		return error;
	}
	ultrawidelock_cred_dynamic_tag_from_ciphertext(ciphertext.data(), dynamicTag.data());
