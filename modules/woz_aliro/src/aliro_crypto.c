			   ...
			   const uint8_t txid[ALIRO_TXID_LEN], uint8_t z[32])
{
	...
	aliro_x963_kdf(shared_secret, ALIRO_SHARED_SECRET_LEN, txid, ALIRO_TXID_LEN, z, 32);
}
			      ...
			      uint8_t block[ALIRO_KEY_BLOCK_LEN])
{
	...
	return aliro_hkdf(salt, salt_len, z, 32, device_pub_x, ALIRO_EC_PUBX_LEN, block,
			  ALIRO_KEY_BLOCK_LEN);
}
...
{
	return aliro_hkdf(salt, salt_len, z, 32, device_pub_x, ALIRO_EC_PUBX_LEN, out, 32);
}
		       ...
		       uint8_t tag[ALIRO_GCM_TAG_LEN])
{
	...
	rc = aliro_aes256_gcm_encrypt(sc->enc_key, nonce, sizeof(nonce), aad, aad_len, pt, pt_len,
				      ct, tag, ALIRO_GCM_TAG_LEN);
	if (rc == 0) {
		sc->enc_ctr++;
	}
	return rc;
		       ...
		       uint8_t *pt)
{
	...
	rc = aliro_aes256_gcm_decrypt(sc->dec_key, nonce, sizeof(nonce), aad, aad_len, ct, ct_len,
				      tag, ALIRO_GCM_TAG_LEN, pt);
	if (rc == 0) {
		sc->dec_ctr++;
	}
	return rc;
				 ...
				 uint8_t ble_device[ALIRO_SESSION_KEY_LEN])
{
	...
	rc = aliro_hkdf(salt, salt_len, ble_sk, 32, (const uint8_t *)"BleSKReader", 11, ble_reader,
			ALIRO_SESSION_KEY_LEN);
	if (rc != 0) {
		return rc;
	}
...
int aliro_salt_build(enum aliro_salt_type type, const uint8_t txid[ALIRO_TXID_LEN],
		     ...
		     uint8_t user_auth_policy, const uint8_t s3opt[ALIRO_EC_PUBX_LEN],
		     const uint8_t *a5_tlv, size_t a5_tlv_len, uint8_t *out, size_t *out_len)
{
	...
	rc |= append(out, &pos, ALIRO_SALT_MAX, k_salt_label + (size_t)type * 12u, 12); /* 2 */
