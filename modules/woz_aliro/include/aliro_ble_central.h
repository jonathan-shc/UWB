...
#include "aliro_advtag.h" /* ALIRO_ADVTAG_LEN */
 ...
 * its own advertised list at 8 (ALIRO_MAX_VERSIONS in aliro_ble.c). */
...
struct aliro_ble_central_peer {
	...
	uint16_t versions[ALIRO_BLE_CENTRAL_MAX_VERSIONS]; /* reader_supported_versions */
 ...
 * Inverse of build_read_payload (aliro_ble.c:271). Returns 0, or -1 if the
 * buffer is short, a length field disagrees with the payload, the version list
 * is odd-length, or it holds more than ALIRO_BLE_CENTRAL_MAX_VERSIONS entries. */
 ...
 * reader itself derives from k_proto_versions (aliro_reader.c:390). Needs
