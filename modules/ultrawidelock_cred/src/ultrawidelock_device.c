...
/* ultrawidelock_apdu_build_authdata, ULTRAWIDELOCK_AUTH_*, INS/TAG */
#include "ultrawidelock_apdu.h"
#include "ultrawidelock_crypto.h" /* z / salt / block / split / gcm nonce */
#include "ultrawidelock_prim.h"   /* EC + AES-GCM primitives */

/* Protocol v1.0 only, matching the reader's ULTRAWIDELOCK_VERSION. Feeds the salt as the
 * protocol_version field (§8.3.1.13), which the reader hardcodes to 0x0100. */
#define ULTRAWIDELOCK_DEV_VERSION 0x0100u
...
/* ---- device AES-256-GCM channel (mirror direction of ultrawidelock_secchan) ---- */
				   ...
				   size_t ct_len, const uint8_t tag[16], uint8_t *pt)
{
	...
	ultrawidelock_crypto_gcm_nonce(0, sc->ctr_r2d, nonce); /* direction 0 = reader->device */
	if (ultrawidelock_aes256_gcm_decrypt(sc->s0, nonce, ULTRAWIDELOCK_GCM_NONCE_LEN, NULL, 0,
					     ct, ct_len, tag, ULTRAWIDELOCK_GCM_TAG_LEN, pt) != 0) {
		return -1;
	}
	sc->ctr_r2d++;
	return 0;
				   ...
				   size_t pt_len, uint8_t *ct, uint8_t tag[16])
{
	...
	ultrawidelock_crypto_gcm_nonce(1, sc->ctr_d2r, nonce); /* direction 1 = device->reader */
	if (ultrawidelock_aes256_gcm_encrypt(sc->s1, nonce, ULTRAWIDELOCK_GCM_NONCE_LEN, NULL, 0,
					     pt, pt_len, ct, tag, ULTRAWIDELOCK_GCM_TAG_LEN) != 0) {
		return -1;
	}
	sc->ctr_d2r++;
	return 0;
 ...
 * ultrawidelock_msg_seal/open framing: a 4-byte [proto][id][len_be16] header (wire length
 * = payload + 16) authenticated as AAD. s0 = BleSKReader (open, dir 0), s1 =
 * BleSKDevice (seal, dir 1). Byte-for-byte inverse of
 * ultrawidelock_msg_seal/ultrawidelock_msg_open.
 */
...
{
	uint8_t ble_reader[ULTRAWIDELOCK_SESSION_KEY_LEN], ble_device[ULTRAWIDELOCK_SESSION_KEY_LEN];

	if (ultrawidelock_crypto_derive_ble_keys(block, versions_salt, salt_len, ble_reader, ble_device) !=
	    0) {
		return -1;
	}
	ultrawidelock_dev_secchan_init(ch, ble_reader, ble_device);
	return 0;
}
...
int ultrawidelock_dev_ble_open(struct ultrawidelock_dev_secchan *ch, const uint8_t *wire,
			       size_t wire_len, uint8_t *plain, size_t plain_cap, size_t *plain_len)
{
	uint8_t nonce[ULTRAWIDELOCK_GCM_NONCE_LEN];
	...
	ultrawidelock_crypto_gcm_nonce(0, ch->ctr_r2d, nonce); /* direction 0 = reader->device */
	if (ultrawidelock_aes256_gcm_decrypt(ch->s0, nonce, ULTRAWIDELOCK_GCM_NONCE_LEN, aad, sizeof(aad),
				     wire + 4u, len_plain, wire + 4u + len_plain, ULTRAWIDELOCK_GCM_TAG_LEN,
				     plain + 4u) != 0) {
		return -1;
	}
			       ...
			       size_t plain_len, uint8_t *wire, size_t wire_cap, size_t *wire_len)
{
	...
	ultrawidelock_crypto_gcm_nonce(1, ch->ctr_d2r, nonce); /* direction 1 = device->reader */
	if (ultrawidelock_aes256_gcm_encrypt(
		    ch->s1, nonce, ULTRAWIDELOCK_GCM_NONCE_LEN, plain, 4u, plain + 4u, len_plain,
		    wire + 4u, wire + 4u + len_plain, ULTRAWIDELOCK_GCM_TAG_LEN) != 0) {
		return -1;
	}
	*wire_len = wl;
	ch->ctr_d2r++;
...
{
	uint8_t iv[ULTRAWIDELOCK_GCM_NONCE_LEN] = {0}; /* §8.3.1.11: 12-byte all-zero IV, no AAD */

	return ultrawidelock_aes256_gcm_encrypt(cryptogram_sk, iv, ULTRAWIDELOCK_GCM_NONCE_LEN,
						NULL, 0, plain, plain_len, out, out + plain_len,
						ULTRAWIDELOCK_GCM_TAG_LEN);
}
				...
				uint8_t block_out[ULTRAWIDELOCK_KEY_BLOCK_LEN])
{
	...
	ultrawidelock_crypto_derive_z(shared_x, txid, z);
	if (ultrawidelock_salt_build(ULTRAWIDELOCK_SALT_SESSION, txid, reader_group_x, reader_eph_x,
				     reader_id, ULTRAWIDELOCK_IFACE_BLE, ULTRAWIDELOCK_DEV_VERSION,
				     exp_phase, 0x01u, NULL, a5, a5n, salt, &slen) != 0) {
		return -1;
	}
	if (ultrawidelock_crypto_derive_block(z, salt, slen, device_eph_x, block) != 0) {
		return -1;
	}
	...
	ultrawidelock_crypto_split(block, 1, enc, dec, ursk);
	ultrawidelock_dev_secchan_init(sc, enc, dec);
	if (block_out != NULL) {
...
/* ---- full initiator state machine (EC via ultrawidelock_prim) ---- */
		      ...
		      const uint8_t reader_id[32], const uint8_t reader_verif_pub[65])
{
	...
	if (ultrawidelock_ec_p256_pub_from_priv(d->cred_priv, d->cred_pub) != 0) {
		return -1;
	}
	...
	d->version = ULTRAWIDELOCK_DEV_VERSION;
	/* Default salt for a peer that publishes v1.0 and nothing else: the list half
	 * and the selected half are both ULTRAWIDELOCK_DEV_VERSION. Correct against our ESP32
	 * reader, wrong against any multi-version peer — see the struct field. */
	d->blesk_salt[0] = (uint8_t)(ULTRAWIDELOCK_DEV_VERSION >> 8);
	d->blesk_salt[1] = (uint8_t)(ULTRAWIDELOCK_DEV_VERSION & 0xffu);
	d->blesk_salt[2] = d->blesk_salt[0];
	d->blesk_salt[3] = d->blesk_salt[1];
	d->blesk_salt_len = 4;
	d->phase = ULTRAWIDELOCK_DEV_IDLE;
	return 0;
}
					...
					size_t len)
{
	...
	if (d == NULL || salt == NULL || len < 4u || (len & 1u) != 0u ||
	    len > ULTRAWIDELOCK_DEV_BLESK_SALT_MAX) {
		return -1;
	}
				    ...
				    size_t len, uint8_t *resp, size_t cap, size_t *resp_len)
{
	...
	if (ultrawidelock_apdu_unwrap(ap_payload, len, &ins, &data, &dlen) != 0) {
		goto fail;
	}

	switch (ins) {
	case ULTRAWIDELOCK_INS_AUTH0: {
		struct ultrawidelock_auth0_command c;

		if (ultrawidelock_dev_parse_auth0_cmd(data, dlen, &c) != 0) {
			goto fail;
		}
		...
		if (ultrawidelock_ec_p256_keygen(d->dev_eph_priv, d->dev_eph_pub) != 0) {
			goto fail;
		}

		size_t n = 0;

		if (ultrawidelock_dev_build_auth0_resp(d->dev_eph_pub, NULL, resp, cap, &n) != 0 ||
		    ultrawidelock_apdu_append_sw(resp, &n, cap, 0x9000u) != 0) {
			goto fail;
		}
		*resp_len = n;
		d->phase = ULTRAWIDELOCK_DEV_SENT_AUTH0_RESP;
		return 0;
	}
	case ULTRAWIDELOCK_INS_AUTH1: {
		struct ultrawidelock_auth1_command c;
		uint8_t td[160], shared[ULTRAWIDELOCK_SHARED_SECRET_LEN], sig[ULTRAWIDELOCK_P256_SIG];
		uint8_t plain[160], tag[ULTRAWIDELOCK_GCM_TAG_LEN];
		uint8_t block[ULTRAWIDELOCK_KEY_BLOCK_LEN];
		size_t tn, pl;

		if (ultrawidelock_dev_parse_auth1_cmd(data, dlen, &c) != 0) {
			goto fail;
		}
		...
		if (ultrawidelock_apdu_build_authdata(ULTRAWIDELOCK_AUTH_READER, d->reader_id, d->dev_eph_pub + 1,
					      d->reader_eph_pub + 1, d->txid, td, sizeof(td),
					      &tn) != 0 ||
		    ultrawidelock_ecdsa_p256_verify(d->reader_verif_pub, td, tn, c.reader_sig) != 0) {
			goto fail;
		}
		/* ECDH + the mirror-image session derivation -> URSK + AP channel. */
		if (ultrawidelock_ecdh_p256(d->dev_eph_priv, d->reader_eph_pub, shared) != 0 ||
		    ultrawidelock_device_derive_session(shared, d->txid, d->reader_group_x,
		 ...
		 * ultrawidelock_device_init defaults it to the v1.0-only 01 00 01 00 and a
		...
		if (d->blesk_salt_len == 0u ||
		    ultrawidelock_dev_blesk_init(&d->sc_ble, block, d->blesk_salt, d->blesk_salt_len) !=
			    0) {
			goto fail;
		}
		...
		if (ultrawidelock_apdu_build_authdata(ULTRAWIDELOCK_AUTH_DEVICE, d->reader_id, d->dev_eph_pub + 1,
					      d->reader_eph_pub + 1, d->txid, td, sizeof(td),
					      &tn) != 0 ||
		    ultrawidelock_ecdsa_p256_sign(d->cred_priv, td, tn, sig) != 0 ||
		    ultrawidelock_dev_build_auth1_resp(sig, d->cred_pub, plain, sizeof(plain), &pl) != 0) {
			goto fail;
		}
		if (cap < pl + ULTRAWIDELOCK_GCM_TAG_LEN + 2u ||
		    ultrawidelock_dev_secchan_seal(&d->sc, plain, pl, resp, tag) != 0) {
			goto fail;
		}
		memcpy(resp + pl, tag, ULTRAWIDELOCK_GCM_TAG_LEN);

		size_t n = pl + ULTRAWIDELOCK_GCM_TAG_LEN;

		if (ultrawidelock_apdu_append_sw(resp, &n, cap, 0x9000u) != 0) {
			goto fail;
		}
		*resp_len = n;
		d->phase = ULTRAWIDELOCK_DEV_SENT_AUTH1_RESP;
		return 0;
	}
	case ULTRAWIDELOCK_INS_EXCHANGE: {
		uint8_t pt[64], plain[8], tag[ULTRAWIDELOCK_GCM_TAG_LEN];
		struct ultrawidelock_exchange_command ec;
		size_t pl;

		if (dlen < ULTRAWIDELOCK_GCM_TAG_LEN) {
			goto fail;
		}

		size_t ctlen = dlen - ULTRAWIDELOCK_GCM_TAG_LEN;

		if (ctlen > sizeof(pt) ||
		    ultrawidelock_dev_secchan_open(&d->sc, data, ctlen, data + ctlen, pt) != 0) {
			goto fail;
		}
		ultrawidelock_dev_parse_exchange_cmd(pt, ctlen, &ec); /* URSK-ready trigger */

		if (ultrawidelock_dev_build_exchange_resp(0x0000u, plain, sizeof(plain), &pl) != 0) {
			goto fail;
		}
		if (cap < pl + ULTRAWIDELOCK_GCM_TAG_LEN + 2u ||
		    ultrawidelock_dev_secchan_seal(&d->sc, plain, pl, resp, tag) != 0) {
		...
		*resp_len = n;
		d->phase = ULTRAWIDELOCK_DEV_ESTABLISHED;
		return 0;
	}
	default:
		break;
...
fail:
	d->phase = ULTRAWIDELOCK_DEV_FAILED;
	return -1;
}
