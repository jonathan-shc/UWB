			      ...
			      size_t *out_len)
{
	...
	*out_len = (size_t)(w.p - out);
	return ULTRAWIDELOCK_STEPUP_OK;
}
					      ...
					      uint8_t *out, size_t cap, size_t *out_len)
{
	...
	cw_map(&iw, 1);
	cw_tstr(&iw, ULTRAWIDELOCK_STEPUP_DOCTYPE_ACCESS); /* namespace "aliro-a" */
	cw_map(&iw, n_elems);
	...
	cw_tstr(&iw, ULTRAWIDELOCK_STEPUP_DOCTYPE_ACCESS);
	if (iw.err) {
		return -1;
	}
 ...
 * ultrawidelock_stepup_build_device_request. Returns ULTRAWIDELOCK_STEPUP_OK, _INVALID_ARGUMENT, or
 * _BUFFER_TOO_SMALL.
 */
 ...
 * returns ULTRAWIDELOCK_STEPUP_INVALID_DATA if the major type or additional-info field does not
 * match expectations.
 */
 ...
 * ULTRAWIDELOCK_STEPUP_INVALID_DATA on format error or invalid argument.
 */
				   ...
				   bool *last_fragment)
{
	...
	*last_fragment = *offset + fragment == encoded_length;
	...
	output[o++] = ULTRAWIDELOCK_INS_ENVELOPE;
	output[o++] = 0x00;
	output[o++] = 0x00;
	if (extended) {
		output[o++] = 0;
				       ...
				       size_t output_capacity, size_t *output_length)
{
	...
	if (expected_length <= 256) {
		...
		const uint8_t apdu[] = {0x00, ULTRAWIDELOCK_INS_GET_RESPONSE, 0x00, 0x00,
		...
		*output_length = sizeof(apdu);
		...
		const uint8_t apdu[] = {0x00, ULTRAWIDELOCK_INS_GET_RESPONSE, 0x00, 0x00, 0x00,
