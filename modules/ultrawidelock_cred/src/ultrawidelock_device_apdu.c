				      ...
				      struct ultrawidelock_auth0_command *c)
{
	...
	if (ultrawidelock_tlv_find(tlv, len, ULTRAWIDELOCK_TAG_EXP_PHASE, &v, &vl) != 0 || vl != 1) {
		return -1;
	}
	c->exp_phase = v[0];
	if (ultrawidelock_tlv_find(tlv, len, ULTRAWIDELOCK_TAG_USER_POL, &v, &vl) != 0 || vl != 1) {
		return -1;
	}
	c->user_policy = v[0];
	if (ultrawidelock_tlv_find(tlv, len, ULTRAWIDELOCK_TAG_VERSION, &v, &vl) != 0 || vl != 2) {
		return -1;
	}
	c->version = (uint16_t)((uint16_t)v[0] << 8 | v[1]);
	if (ultrawidelock_tlv_find(tlv, len, ULTRAWIDELOCK_TAG_READER_EPH, &v, &vl) != 0 || vl != 65) {
		return -1;
	}
	memcpy(c->reader_eph_pub, v, 65);
	if (ultrawidelock_tlv_find(tlv, len, ULTRAWIDELOCK_TAG_TXID, &v, &vl) != 0 || vl != 16) {
		return -1;
	}
	memcpy(c->txid, v, 16);
	if (ultrawidelock_tlv_find(tlv, len, ULTRAWIDELOCK_TAG_READER_ID, &v, &vl) != 0 || vl != 32) {
		return -1;
	}
				      ...
				      struct ultrawidelock_auth1_command *c)
{
	...
	if (ultrawidelock_tlv_find(tlv, len, ULTRAWIDELOCK_TAG_SIG, &v, &vl) != 0 || vl != 64) {
					 ...
					 struct ultrawidelock_exchange_command *c)
{
	...
	memset(c, 0, sizeof(*c));
	if (ultrawidelock_tlv_find(plain, len, ULTRAWIDELOCK_TAG_STATUS, &v, &vl) == 0 && vl == 2) {
		c->have_status = 1;
		c->reader_status = (uint16_t)((uint16_t)v[0] << 8 | v[1]);
	}
	if (ultrawidelock_tlv_find(plain, len, ULTRAWIDELOCK_TAG_URSK_READY, &v, &vl) == 0) {
		c->ursk_ready = 1;
	}
	return 0;
}
				       ...
				       size_t *out_len)
{
	...
	ultrawidelock_tlv_put(&w, ULTRAWIDELOCK_TAG_DEVICE_PUBX, device_eph_pub, 65);
	if (cryptogram64) {
		ultrawidelock_tlv_put(&w, 0x9Du, cryptogram64, 64);
	}
			       ...
			       uint8_t *out, size_t cap, size_t *out_len)
{
	...
	ultrawidelock_tlv_w_init(&w, out, cap);
	ultrawidelock_tlv_put(&w, ULTRAWIDELOCK_TAG_SIG, device_sig, 64);
	if (device_pub65) {
		ultrawidelock_tlv_put(&w, ULTRAWIDELOCK_TAG_DEVICE_PUB, device_pub65, 65);
	}
	return ultrawidelock_tlv_w_finish(&w, out_len);
}
 ...
 * big-endian format.
 */
int ultrawidelock_dev_build_exchange_resp(uint16_t error, uint8_t *out, size_t cap, size_t *out_len)
{
