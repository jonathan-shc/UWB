...
static void slope_track(struct ultrawidelock_rssi_gate *g,
			const struct ultrawidelock_rssi_gate_cfg *cfg, uint32_t now_ms)
{
 ...
 * (ULTRAWIDELOCK_RSSI_UNAVAILABLE) are skipped and do not alter the state.
 */
bool ultrawidelock_rssi_gate_feed(struct ultrawidelock_rssi_gate *g,
				  ...
				  uint32_t now_ms)
{
	if (rssi_dbm == ULTRAWIDELOCK_RSSI_UNAVAILABLE) {
		return g->open;
	}
