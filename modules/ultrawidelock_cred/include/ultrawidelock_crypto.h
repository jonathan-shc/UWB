 ...
 * into ultrawidelock_uwb_start_cred(cfg) is 3.3.
 */
#pragma once
...
#define ULTRAWIDELOCK_KEY_BLOCK_LEN     160u /* full derived block */
#define ULTRAWIDELOCK_URSK_OFFSET       128u /* URSK = block[128 .. 159] */
#define ULTRAWIDELOCK_SESSION_KEY_LEN   32u
#define ULTRAWIDELOCK_SHARED_SECRET_LEN 32u /* ECDH X coordinate */
#define ULTRAWIDELOCK_TXID_LEN          16u
#define ULTRAWIDELOCK_EC_PUBX_LEN       32u /* an EC point's X coordinate */
#define ULTRAWIDELOCK_GCM_NONCE_LEN     12u
#define ULTRAWIDELOCK_GCM_TAG_LEN       16u
...
#define ULTRAWIDELOCK_IFACE_NFC 0x5Eu
#define ULTRAWIDELOCK_IFACE_BLE 0xC3u
 ...
 * salt is the CreateSalt transcript (ultrawidelock_salt_build). Returns 0 on success.
 */
 ...
 *     ultrawidelock_crypto_derive_key32(IKM = Kdh, salt = salt_persistent, info =
 *     credential_ephemeral_pub_x) -> 32 bytes. salt_persistent is ultrawidelock_salt_build
 *     type ULTRAWIDELOCK_SALT_KPERSISTENT, which appends the Access Credential public-key X.
 *
 *   Fast block (per §8.3.1.12):
 *     ultrawidelock_crypto_derive_block(IKM = Kpersistent, salt = salt_fast, info =
 *     credential_ephemeral_pub_x) -> 160 bytes. salt_fast is ultrawidelock_salt_build type
 *     ULTRAWIDELOCK_SALT_CRYPTOGRAM ("VolatileFast"), also appending the credential pub-key X.
 *
 ...
 * URSK@128. So BleSK (ultrawidelock_crypto_derive_ble_keys) and URSK (offset 128) come
 * out exactly as in the standard block, and ultrawidelock_crypto_split(block, 0, ...)
 * yields ExpeditedSKReader (enc) / ExpeditedSKDevice (dec). CryptogramSK is the
 * leading 32 bytes.
 */
#define ULTRAWIDELOCK_CRYPTOGRAM_SK_OFFSET 0u  /* CryptogramSK = fast_block[0 .. 31] */
#define ULTRAWIDELOCK_CRYPTOGRAM_LEN 64u /* AUTH0 response tag 0x9D: enc_payload(48) || tag(16) */
 ...
 */
struct ultrawidelock_secchan {
	uint8_t enc_key[ULTRAWIDELOCK_SESSION_KEY_LEN];
	uint8_t dec_key[ULTRAWIDELOCK_SESSION_KEY_LEN];
	uint32_t enc_ctr;
	uint32_t dec_ctr;
};
 ...
 * where len_be16 = plaintext length + 16. Reuse struct ultrawidelock_secchan for it
 * (enc=BleSKReader, dec=BleSKDevice; ultrawidelock_secchan_init sets both counters to 1).
 */
#define ULTRAWIDELOCK_BLESK_OFFSET 96u /* BleSK = block[96 .. 127] (§8.3.1.12/.13) */
...
/* Inverse of ultrawidelock_msg_seal: open a wire SDU into the engine plaintext form,
 ...
 */
enum ultrawidelock_salt_type {
	ULTRAWIDELOCK_SALT_CRYPTOGRAM = 0, /* label "VolatileFast" */
	ULTRAWIDELOCK_SALT_SESSION = 1,    /* label "Volatile****" */
	ULTRAWIDELOCK_SALT_KPERSISTENT = 2 /* label "Persistent**" */
};

#define ULTRAWIDELOCK_SALT_MAX 256u
...
int ultrawidelock_salt_build(enum ultrawidelock_salt_type type,
			     ...
			     const uint8_t s3opt[ULTRAWIDELOCK_EC_PUBX_LEN] /* NULL for type 1 */,
