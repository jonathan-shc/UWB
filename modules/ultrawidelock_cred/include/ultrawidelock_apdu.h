...
#define ULTRAWIDELOCK_PROTO_ACCESS       0x00u /* Access Protocol: payload is an ISO7816 APDU */
#define ULTRAWIDELOCK_PROTO_NOTIFICATION 0x02u /* Notification: Initiate-AP / Event */

/* ---- Access-Protocol opcode (envelope byte 1, directional) ---- */
#define ULTRAWIDELOCK_AP_OP_COMMAND  0x00u /* reader -> device APDU command */
#define ULTRAWIDELOCK_AP_OP_RESPONSE 0x01u /* device -> reader APDU response (<tlv> SW1 SW2) */

/* ---- Notification opcode (envelope byte 1 when type == NOTIFICATION) ---- */
#define ULTRAWIDELOCK_NOTIF_EVENT       0x00u /* Event, e.g. GeneralError [01 01 <code>] */
#define ULTRAWIDELOCK_NOTIF_INITIATE_AP 0x05u /* Initiate Access Protocol (phone's first msg) */
...
#define ULTRAWIDELOCK_INS_AUTH0    0x80u
#define ULTRAWIDELOCK_INS_AUTH1    0x81u
#define ULTRAWIDELOCK_INS_EXCHANGE 0xC9u

/* ---- TLV tags ---- */
#define ULTRAWIDELOCK_TAG_EXP_PHASE                                                                \
	0x41u /* AUTH0: ExpeditedPhaseType; AUTH1: AccessCredentialType */
#define ULTRAWIDELOCK_TAG_USER_POL   0x42u /* AUTH0: UserAuthenticationPolicy */
#define ULTRAWIDELOCK_TAG_VERSION    0x5Cu /* AUTH0: protocol version u16 BE */
#define ULTRAWIDELOCK_TAG_READER_EPH 0x87u /* reader ephemeral pubkey (65) / transcript pubX (32) */
#define ULTRAWIDELOCK_TAG_TXID       0x4Cu /* transaction identifier (16) */
#define ULTRAWIDELOCK_TAG_READER_ID  0x4Du /* reader identifier (32) */
#define ULTRAWIDELOCK_TAG_SIG        0x9Eu /* ECDSA signature r|s (64) */
#define ULTRAWIDELOCK_TAG_DEVICE_PUBX                                                              \
	0x86u /* transcript device pubX (32) / AUTH0Resp device eph pub (65) */
#define ULTRAWIDELOCK_TAG_USAGE      0x93u /* transcript usage domain separator (4) */
#define ULTRAWIDELOCK_TAG_DEVICE_PUB 0x5Au /* AUTH1Resp device public key (65) */
#define ULTRAWIDELOCK_TAG_STATUS     0x97u /* EXCHANGE ReaderStatus (u16 BE) */
#define ULTRAWIDELOCK_TAG_URSK_READY 0x98u /* EXCHANGE URSK-ready trigger (zero length) */

/* ---- BER-TLV writer ---- */
struct ultrawidelock_tlv_w {
	uint8_t *buf;
	size_t cap;
	size_t len;
	int err;
...
void ultrawidelock_tlv_put_u16(struct ultrawidelock_tlv_w *w, uint8_t tag,
			       uint16_t v);                                   /* big-endian */
void ultrawidelock_tlv_put_empty(struct ultrawidelock_tlv_w *w, uint8_t tag); /* zero length */
int ultrawidelock_tlv_w_finish(struct ultrawidelock_tlv_w *w,
			       size_t *out_len); /* 0 ok, -1 overflow */
...
#define ULTRAWIDELOCK_AUTH_READER 1
#define ULTRAWIDELOCK_AUTH_DEVICE 0
 ...
 * (Le = 0x00 => up to 256 response bytes). ins is one of ULTRAWIDELOCK_INS_*. The result
...
struct ultrawidelock_auth0_response {
...
struct ultrawidelock_auth1_response {
...
#define ULTRAWIDELOCK_ENVELOPE_HDR 4u
