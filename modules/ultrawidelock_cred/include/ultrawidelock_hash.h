...
// calls.
/*
 * ultrawidelock_hash — portable SHA-256 and its KDF constructions (HMAC-SHA256,
...
#define ULTRAWIDELOCK_SHA256_BLOCK 64u
#define ULTRAWIDELOCK_SHA256_LEN   32u

/* Streaming SHA-256 (FIPS 180-4). */
struct ultrawidelock_sha256 {
	uint32_t h[8];
	uint64_t total; /* message length in bytes */
	uint8_t buf[ULTRAWIDELOCK_SHA256_BLOCK];
	size_t buflen;
};
