/**
 * @file woz_side_log.c — pack/check bounded side-decision records.
 */
		      ...
		      struct woz_side_log_rec *out)
{
	...
	out->magic = WOZ_SIDE_LOG_MAGIC;
	out->version = WOZ_SIDE_LOG_VERSION;
	...
	out->crc16 = woz_side_log_crc16(out, offsetof(struct woz_side_log_rec, crc16));
...
int woz_side_log_check(const struct woz_side_log_rec *rec)
{
	...
	if (rec->magic != WOZ_SIDE_LOG_MAGIC || rec->version != WOZ_SIDE_LOG_VERSION) {
		return -1;
	}
	crc = woz_side_log_crc16(rec, offsetof(struct woz_side_log_rec, crc16));
