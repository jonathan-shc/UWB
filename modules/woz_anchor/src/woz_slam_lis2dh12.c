/**
 * @file woz_slam_lis2dh12.c — LIS2DH12 impact interrupt, raw registers (Zephyr).
 *
...
LOG_MODULE_REGISTER(woz_slam, CONFIG_WOZ_ANCHOR_SLAM_LOG_LEVEL);
...
int woz_slam_hw_init(void)
{
	...
	/* CONFIG_WOZ_ANCHOR_SLAM_THRESHOLD_MG is a placeholder until a bench
	...
	ths = (uint8_t)(CONFIG_WOZ_ANCHOR_SLAM_THRESHOLD_MG / THS_MG_PER_LSB);
	if (ths > THS_MAX) {
		ths = THS_MAX;
	}
	if (ths == 0u) {
	...
	LOG_INF("lis2dh12: impact interrupt armed at %u mg", CONFIG_WOZ_ANCHOR_SLAM_THRESHOLD_MG);
	return 0;
}

bool woz_slam_hw_take(void)
{
