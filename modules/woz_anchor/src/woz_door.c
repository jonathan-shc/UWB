		   ...
		   const struct woz_door_thresholds *th)
{
	...
	if (th != NULL) {
		...
		woz_door_defaults(&d->th);
	}
	if (d->th.dwell == 0u) {
		d->th.dwell = 1u;
	}
	d->state = WOZ_DOOR_UNKNOWN;
	d->cand = WOZ_DOOR_UNKNOWN;
	d->cand_n = 0u;
	d->last_mddeg = WOZ_DOOR_ANGLE_INVALID;
	return true;
}
...
{
	switch (d->state) {
	case WOZ_DOOR_CLOSED:
		if (mddeg <= d->th.closed_leave_mddeg) {
			return WOZ_DOOR_CLOSED;
		}
		break;
	case WOZ_DOOR_OPEN:
		if (mddeg >= d->th.open_leave_mddeg) {
			return WOZ_DOOR_OPEN;
		}
		break;
	default:
		break;
	...
	return WOZ_DOOR_AJAR;
}

enum woz_door_state woz_door_feed(struct woz_door *d, int32_t d_mm)
{
	int32_t mddeg;
	enum woz_door_state want;

	if (d == NULL) {
		return WOZ_DOOR_UNKNOWN;
	}
	mddeg = woz_door_angle_mddeg(&d->cfg, d_mm);
	d->last_mddeg = mddeg;
	if (mddeg == WOZ_DOOR_ANGLE_INVALID) {
		/*
