/**
 * @file woz_side.h — safety-oriented INSIDE/OUTSIDE/THRESHOLD/UNKNOWN gate
 *       for passive approach unlock.
 *
 * Separate from woz_fusion_may_predict(): that legacy helper fail-opens on
...
/** Classifier / policy side labels. THRESHOLD extends enum woz_side. */
enum woz_side_label {
	WOZ_SIDE_LABEL_UNKNOWN = WOZ_SIDE_UNKNOWN,
	WOZ_SIDE_LABEL_INSIDE = WOZ_SIDE_INSIDE,
	WOZ_SIDE_LABEL_OUTSIDE = WOZ_SIDE_OUTSIDE,
	WOZ_SIDE_LABEL_THRESHOLD = 3,
};

/** Temporal approach state around the door plane. */
enum woz_side_motion {
	WOZ_SIDE_MOTION_UNKNOWN = 0,
	WOZ_SIDE_MOTION_OUTSIDE_FAR,
	WOZ_SIDE_MOTION_OUTSIDE_APPROACHING,
	WOZ_SIDE_MOTION_OUTSIDE_NEAR,
	WOZ_SIDE_MOTION_THRESHOLD,
	WOZ_SIDE_MOTION_INSIDE_NEAR,
	WOZ_SIDE_MOTION_INSIDE_FAR,
};

/** Bitmask of contributors that fed the latest decision. */
enum woz_side_anchor_mask {
	WOZ_SIDE_ANCHOR_PRIMARY_UWB = 1u << 0,
	WOZ_SIDE_ANCHOR_BLE_INSIDE = 1u << 1,
	WOZ_SIDE_ANCHOR_BLE_OUTSIDE = 1u << 2,
	WOZ_SIDE_ANCHOR_BLE_THRESHOLD = 1u << 3,
	WOZ_SIDE_ANCHOR_UWB_SATELLITE = 1u << 4,
	WOZ_SIDE_ANCHOR_DOOR = 1u << 5,
};

/** Compact observation features for one correlated window. */
struct woz_side_features {
	...
	uint8_t door_state; /**< WOZ_DOOR_* or 0 = unknown */
	uint8_t prev_side;  /**< last confirmed WOZ_SIDE_LABEL_* */
	uint16_t since_confirmed_ms;

	uint8_t classifier_ver;
	uint8_t calibration_ver;
	uint8_t anchor_health_mask; /**< bits of enum woz_side_anchor_mask that are healthy */
	uint8_t flags;              /**< WOZ_SIDE_F_* */
};

/** Feature / decision flags. */
#define WOZ_SIDE_F_DEGRADED           0x01u
#define WOZ_SIDE_F_MULTI_PHONE        0x02u
#define WOZ_SIDE_F_SESSION_MISMATCH   0x04u
#define WOZ_SIDE_F_REPLAY             0x08u
#define WOZ_SIDE_F_VERSION_MISMATCH   0x10u
#define WOZ_SIDE_F_INSIDE_CONTRADICT  0x20u
#define WOZ_SIDE_F_EVIDENCE_STALE     0x40u
#define WOZ_SIDE_F_QUORUM_FAIL        0x80u
...
struct woz_side_filter {
	struct woz_side_cfg cfg;
	enum woz_side_label cand;
	enum woz_side_label committed;
	enum woz_side_motion motion;
	uint8_t cand_n;
	uint8_t confidence;
