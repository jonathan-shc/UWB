...
/**
 * @file ultrawidelock_side_log.h — bounded binary record for side features and decisions.
 *
...
#ifndef ULTRAWIDELOCK_SIDE_LOG_H
#define ULTRAWIDELOCK_SIDE_LOG_H
...
#define ULTRAWIDELOCK_SIDE_LOG_MAGIC   0x5344u /* 'SD' */
#define ULTRAWIDELOCK_SIDE_LOG_VERSION 1u
#define ULTRAWIDELOCK_SIDE_LOG_SIZE    48u

/** Packed on-wire / on-UART record. */
struct ultrawidelock_side_log_rec {
	uint16_t magic;
	uint8_t version;
	uint8_t side;
	uint8_t motion;
...
_Static_assert(sizeof(struct ultrawidelock_side_log_rec) == ULTRAWIDELOCK_SIDE_LOG_SIZE,
	       "ultrawidelock_side_log_rec size");
...
#endif /* ULTRAWIDELOCK_SIDE_LOG_H */
