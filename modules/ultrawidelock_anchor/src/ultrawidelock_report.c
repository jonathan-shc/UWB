/**
 * @file ultrawidelock_report.c — range report line codec (implementation).
 *
...
int ultrawidelock_report_format(const struct ultrawidelock_range_report *r, char *out, size_t cap)
{
	...
	app_str(&a, ULTRAWIDELOCK_REPORT_MAGIC);
	app_ch(&a, ' ');
	...
	crc = ultrawidelock_report_crc16(out, a.len);
	app_ch(&a, ' ');
	app_ch(&a, '*');
	if (a.fail) {
		return -ENOSPC;
...
int ultrawidelock_report_parse(const char *line, size_t len, struct ultrawidelock_range_report *out)
{
	 ...
	 */
	if (memcmp(line, ULTRAWIDELOCK_REPORT_MAGIC, MAGIC_LEN) != 0) {
		return -EBADMSG;
	}
	...
	want = ultrawidelock_report_crc16(line, covered);
	if (want != got) {
		return -EBADMSG;
	}
