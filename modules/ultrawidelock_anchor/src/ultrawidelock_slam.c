...
/**
 * @file ultrawidelock_slam.c — impact and tamper classification (implementation).
 */
						      ...
						      bool struck, int64_t now_ms)
{
	...
	if (cfg == NULL || s == NULL || cfg->tamper_count == 0u || !struck) {
		return ULTRAWIDELOCK_SLAM_NONE;
	}
	...
	if (s->window_count >= cfg->tamper_count) {
		...
		if (!s->tamper_latched) {
			...
			return ULTRAWIDELOCK_SLAM_TAMPER;
		}
		return ULTRAWIDELOCK_SLAM_NONE;
	}

	return ULTRAWIDELOCK_SLAM_IMPACT;
}
