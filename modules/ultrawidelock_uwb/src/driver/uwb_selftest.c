...
static void uwb_selftest_work(struct ultrawidelock_dwork *dwork)
{
	...
	const struct ultrawidelock_uwb_aliro_cfg cfg = {
	...
	LOG_INF("SELFTEST: ultrawidelock_uwb_start_aliro() [responder-RX wrap probe] ...");
	rc = ultrawidelock_uwb_start_aliro(&cfg);
	LOG_INF("SELFTEST: ultrawidelock_uwb_start_aliro() = %d %s", rc,
