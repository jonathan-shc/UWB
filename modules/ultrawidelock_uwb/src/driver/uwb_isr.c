...
#include "trace.h"
/* ultrawidelock_uwb_arm_rx / ultrawidelock_uwb_set_callbacks — the engine's seam */
#include "uwb_seam.h"
...
/** @brief RX-good callback: peek the frame header, log via ULTRAWIDELOCK_TRACE, then re-arm RX. */
static void cb_rx_ok(const dwt_cb_data_t *d)
{
	...
	(void)ultrawidelock_uwb_arm_rx(DWT_START_RX_IMMEDIATE);
}

/** @brief RX-timeout callback: frame-wait window expired; re-arms RX. */
static void cb_rx_to(const dwt_cb_data_t *d)
{
	ULTRAWIDELOCK_TRACE("uwb.rx.to", "status=0x%08X", (unsigned int)d->status);
	(void)ultrawidelock_uwb_arm_rx(DWT_START_RX_IMMEDIATE);
}
...
{
	ULTRAWIDELOCK_TRACE("uwb.rx.err", "status=0x%08X", (unsigned int)d->status);
	(void)ultrawidelock_uwb_arm_rx(DWT_START_RX_IMMEDIATE);
}
...
{
	ULTRAWIDELOCK_TRACE("uwb.tx.done", "status=0x%08X", (unsigned int)d->status);
}
...
int uwb_isr_register(void)
{
	...
	ultrawidelock_uwb_set_callbacks(&callbacks);
