...
enum fr_rec_type {
	...
	FR_REC_CONFIG = 2, /* the woz_uwb_aliro_cfg that opened the session */
 ...
 * ccc_shim_rx.c / woz_uwb_facade.c cost nothing in a hardened build. */
#if defined(ESP_PLATFORM)
#include "sdkconfig.h" /* CONFIG_WOZ_FLIGHT_RECORDER (Zephyr injects autoconf.h itself) */
#endif

struct woz_uwb_aliro_cfg; /* forward decl; the real def is in woz_uwb_facade.h */
...
void fr_capture_config(const struct woz_uwb_aliro_cfg *cfg);          /* session-start hook */
