...
- **Don't** invent a hex for a state that already has a semantic alias. Mint,
  ...
  primitive.
- **Don't** add a token before checking the five that are still declared and
  unreferenced: `--fs-1100`, `--article`, `--r-xl`, `--sp-40`, `--ease-decel`.
  `--reveal-step` now carries the scroll stagger and `--ease-spring` carries the
  verdict stamp in the landing entrance.
