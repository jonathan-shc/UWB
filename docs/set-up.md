...
2. The NCS v3.3.0 toolchain, via `nrfutil sdk-manager toolchain install`. An
   ...
   have used. If yours is managed some other way, `ULTRAWIDELOCK_TOOLCHAIN=env` uses
...
| `ULTRAWIDELOCK_WS=/big/disk/ws` | put the workspace somewhere else |
| `ULTRAWIDELOCK_SHALLOW=1` | shallow fetch, saves several GB (what CI uses) |
| `ULTRAWIDELOCK_TOOLCHAIN=env` | use the toolchain already on `PATH` |
...
toolchain already on `PATH` (`ULTRAWIDELOCK_TOOLCHAIN=env`), which skips phase 2 too.
...
no cable and no probe. Details: [`apps/dwm3001cdk-lock/README.md`](../apps/dwm3001cdk-lock/README.md).
...
```bash
git config git-pr.verify make check
```
...
**Reader** (`../examples/esp32/reader`): plain ESP-IDF, no esp-matter.
...
**Matter door lock** (`../apps/esp32-matter-lock`): also needs
...
Running these from the app directory still works — `cd examples/esp32/reader
