...
ALIRO_UUID = "0000fff2-0000-1000-8000-00805f9b34fb"
...
async def find():
    return await BleakScanner.find_device_by_filter(
        lambda d, a: ALIRO_UUID in [u.lower() for u in (a.service_uuids or [])],
        timeout=15.0,
    )
...
async def main(scan_only):
    dev = await find()
    if dev is None:
        bad("no advertisement carrying the Aliro service UUID 0xFFF2")
        return 2
    ...
    async with BleakClient(dev, timeout=20.0) as client:
        ...
        if ALIRO_UUID in uuids:
            ok("Aliro GATT service is discoverable")
        else:
            bad("Aliro GATT service missing from the table")
            fails += 1
        if DFU_SVC in uuids:
