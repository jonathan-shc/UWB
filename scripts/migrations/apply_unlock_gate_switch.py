...
# PREDICT falls through into THRESHOLD after any anchor veto, so this single
# hook gates both passive unlock actions while leaving explicit Home/NFC paths alone.
main = MAIN.read_text()
if 'passive unlock withheld by owner gate' not in main:
    needle = '\t\tcase ULTRAWIDELOCK_APPROACH_UNLOCK_THRESHOLD:\n'
    if needle not in main:
        raise SystemExit('passive unlock threshold anchor not found in main.c')
    ...
    MAIN.write_text(main.replace(needle, gate, 1))
...
# PartsList: find the ROOT descriptor implementation, not endpoint-1 descriptor.
root_parts = c.find('MATTER_ENDPOINT_LOCK', c.find('case MATTER_ATTR_DESC_PARTS_LIST:'))
if 'MATTER_ENDPOINT_UNLOCK_GATE' not in c[c.find('case MATTER_ATTR_DESC_PARTS_LIST:'):c.find('static void lock_attr_value')]:
    anchor = '\t\t\t(void)matter_tlv_put_u64(w, MATTER_TLV_ANON, MATTER_ENDPOINT_LOCK);\n'
    if anchor not in c:
        raise SystemExit('root PartsList anchor not found')
    c = c.replace(anchor, anchor + '#if IS_ENABLED(CONFIG_ULTRAWIDELOCK_UNLOCK_GATE_SWITCH)\n\t\t\t(void)matter_tlv_put_u64(w, MATTER_TLV_ANON, MATTER_ENDPOINT_UNLOCK_GATE);\n#endif\n', 1)
...
CLU.write_text(c)
print('Standalone unlock gate integration applied.')
print('No battery files or symbols are required.')
