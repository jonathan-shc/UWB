...
Sources, and there is exactly one of each on purpose:
    ...
    verifier, salt, iterations, discriminator
        ports/freertos-nrf52833/matter/matter_compat/woz_freertos_matter_config.h
    passcode
        apps/dwm3001cdk-lock/Kconfig, ALIRO_MATTER_SETUP_PASSCODE
...
def header_defines(path):
    ...
    m = re.search(r"#define\s+CONFIG_ALIRO_MATTER_DISCRIMINATOR\s+(0[xX][0-9a-fA-F]+|\d+)", text)
    if m:
        out["discriminator"] = int(m.group(1), 0)

    m = re.search(r"#define\s+CONFIG_ALIRO_MATTER_SPAKE2P_ITERATIONS\s+(\d+)", text)
    if m:
        out["iterations"] = int(m.group(1))

    for name, key in (("SPAKE2P_VERIFIER", "verifier"), ("SPAKE2P_SALT", "salt")):
        m = re.search(r"#define\s+CONFIG_ALIRO_MATTER_" + name + r"\b(.*?)(?=#define|\Z)",
                      text, re.S)
        if m:
...
def kconfig_passcode(path):
    text = open(path).read()
    m = re.search(r"config\s+ALIRO_MATTER_SETUP_PASSCODE\b(.*?)(?=\nconfig\s|\Z)", text, re.S)
    if not m:
        return None
