...
PAIRS = {
    ...
    # The four below were added after the fact, because each had already drifted
    # and three of them cost a target compile to find: an RTC span macro that
    # existed only in the fake, an NVMC partial-erase mode this part does not
    # have, and a TIMER header that one commit replaced with a narrower one and
    # left the vendor high-precision timer gate unable to compile.
    "tests/ports/freertos-nrf52833/fake/hal/nrf_rtc.h": NRFX + "/hal/nrf_rtc.h",
    "tests/ports/freertos-nrf52833/fake/hal/nrf_nvmc.h": NRFX + "/hal/nrf_nvmc.h",
    "tests/ports/freertos-nrf52833/fake/hal/nrf_timer.h": NRFX + "/hal/nrf_timer.h",
    "tests/ports/freertos-nrf52833/fake/hal/nrf_rng.h": NRFX + "/hal/nrf_rng.h",
}
