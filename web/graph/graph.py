...
# regenerated by `python3 web/build.py` whenever graphify-out/graph.json is
# present, so it is refreshed as a side effect of building, and the diff shows
# up in review rather than silently rotting.
...
def distil(graph: dict) -> dict:
    ...
    return {
        ...
        "note": ("Generated from graphify-out/graph.json by web/build.py. "
                 "Refresh with `graphify update .` then `make docs`."),
...
def write_distillate(graph: dict, dest: Path = DISTILLATE) -> bool:
    ...
    body = json.dumps(distil(graph), indent=1) + "\n"
