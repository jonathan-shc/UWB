...
# The committed distillate. graphify's own graph.json is 11 MB of 7,969 nodes
# and 18,457 edges; this page aggregates all of it down to 17 subsystems and 49
# edges, which is 3 KB. So the 3 KB is what the repository carries, and the
# 11 MB stays a machine-local artifact of a tool that is not in this tree.
#
# That is the difference between this and the twin.js mistake the README names:
# this file is small, sorted, one entry per line, and a reviewer can read what
# changed -- a subsystem gained four files, a new dependency appeared. It is
# regenerated by `make docs-graph-refresh`, on purpose and never as a side
# effect of an ordinary build: line one is the commit the graph was extracted
# at, so a build that rewrote it left every worktree dirty and put a guaranteed
# conflict on that line between any two branches that had both built.
DISTILLATE = Path(__file__).with_name("subsystems.json")


def distil(graph: dict) -> dict:
    """Reduce a full graphify graph to what this page actually draws."""
    sizes, edges = aggregate(graph)
    return {
        "built_at_commit": graph.get("built_at_commit", "unknown"),
        "note": ("Generated from graphify-out/graph.json by web/build.py. "
                 "Refresh with `graphify update .` then "
                 "`make docs-graph-refresh`."),
        "subsystems": {k: sizes[k] for k in sorted(sizes)},
        "edges": [[a, b, edges[(a, b)]] for (a, b) in sorted(edges)],
    }


def distillate_is_current(graph: dict, dest: Path = DISTILLATE) -> bool:
    """Whether the committed file already says what this graph says."""
    return (dest.is_file()
            and dest.read_text(encoding="utf-8") == _distillate_text(graph))


def write_distillate(graph: dict, dest: Path = DISTILLATE) -> bool:
    """Write the distillate, returning whether it changed."""
    if distillate_is_current(graph, dest):
        return False
    dest.write_text(_distillate_text(graph), encoding="utf-8")
    return True


def _distillate_text(graph: dict) -> str:
    """The distillate's exact on-disk bytes. One definition, so that the
    staleness check and the write can never disagree about formatting."""
    return json.dumps(distil(graph), indent=1) + "\n"


def load(source: Path) -> tuple[Counter, Counter, str]:
    """Accept either a full graphify graph or the committed distillate."""
    data = json.loads(source.read_text())
    if "subsystems" in data:
        sizes = Counter(data["subsystems"])
        edges = Counter({(a, b): n for a, b, n in data["edges"]})
        return sizes, edges, data.get("built_at_commit", "unknown")
    sizes, edges = aggregate(data)
    return sizes, edges, data.get("built_at_commit", "unknown")


def aggregate(graph: dict) -> tuple[Counter, Counter]:
    """Subsystem -> distinct source files, and subsystem pair -> references.

    Sizes count *files*, not nodes, because the page labels them "n files" and
    that has to be true. graphify emits one node per extracted symbol, so
    counting nodes reported modules/ultrawidelock_cred as 504 files when it
    holds a few dozen. Edges still count node-level references, which is what
    the line weight means and what the footnote says it means.
    """
    owner: dict[str, str] = {}
    ...
    for node in graph["nodes"]:
        ...
        if name:
            owner[node["id"]] = name
            files.setdefault(name, set()).add(src)
    sizes: Counter = Counter({name: len(paths) for name, paths in files.items()})
...
def render(graph_json: Path) -> str:
    sizes, all_edges, commit = load(graph_json)
    if not sizes:
    ...
    hue_of = {name: HUES[TIERS.index(name.split("/")[0])] for name in sorted(sizes)}
