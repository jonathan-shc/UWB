 ...
 *
 * Compiled (emcc) with the untouched modules/ultrawidelock_uwb sources plus the same
...
#include "ultrawidelock_crypto.h" /* ULTRAWIDELOCK_URSK_LEN */
#include "ccc_shim.h"
...
/**
 * Increment the digital twin's latch counter on each successful unlock decision.
 */
...
EMSCRIPTEN_KEEPALIVE int twin_boot(void)
{
	...
	ultrawidelock_uwb_set_range_listener(twin_on_latch);
...
EMSCRIPTEN_KEEPALIVE int twin_step(int cm)
{
	...
	case 0: /* Pre-POLL: stash (decode deferred to the idle) + arm event */
		...
		twin_stash_frame(frame, len, g_ts);
		ccc_shim_rx_try_prepoll(len);
		twin_rx_event(ultrawidelock_host_rx.cbs.cbRxOk, TWIN_ST_GOOD);
		break;
	case 1: /* POLL result (cper=0): t2 in, Response_0 delayed TX out */
		g_t2 = g_ts + 0x10000ull;
		ultrawidelock_host_rx.rx_ts40 = g_t2;
		twin_rx_event(ultrawidelock_host_rx.cbs.cbRxOk, DWT_INT_CIADONE_BIT_MASK);
		break;
	case 2: /* Response TXFRS: t3 = t2 + reply1; Final window armed */
		ultrawidelock_host_rx.tx_ts40 = g_t2 + TWIN_REPLY1;
		twin_rx_event(ultrawidelock_host_rx.cbs.cbTxDone, DWT_INT_TXFRS_BIT_MASK);
		break;
	case 3: /* Final RFRAME: t6 = t3 + round2, good STS; revert to SP0 */
		ultrawidelock_host_rx.rx_ts40 = g_t2 + TWIN_REPLY1 + TWIN_ROUND2;
...
EMSCRIPTEN_KEEPALIVE int twin_last_cm(void)
{
	...
	return ultrawidelock_uwb_last_range_cm(&cm) ? (int)cm : TWIN_NO_RANGE;
}
...
EMSCRIPTEN_KEEPALIVE int twin_trusted_cm(void)
{
	...
	return ultrawidelock_uwb_trusted_range_cm(&cm) ? (int)cm : TWIN_NO_RANGE;
}
...
{
	return ultrawidelock_host_rx.rxenable_calls;
}
...
{
	return ultrawidelock_host_rx.starttx_calls;
}
