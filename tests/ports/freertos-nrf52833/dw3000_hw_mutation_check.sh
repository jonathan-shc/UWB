...
MUTATIONS=(
 ...
 ::: "
	...
	"the vector runs one level below the frozen priority ::: #define ULTRAWIDELOCK_DW3000_IRQ_PRIORITY 4u ::: #define ULTRAWIDELOCK_DW3000_IRQ_PRIORITY 5u"
 ...
 ::: "
	...
	"the worker services one event instead of draining the line ::: 		while (nrf_gpio_pin_read(ULTRAWIDELOCK_DW3000_PIN_IRQ)) {
			dwt_isr();
		} ::: 		dwt_isr();"
	"the worker runs at an ordinary priority ::: #define ULTRAWIDELOCK_DW3000_ISR_TASK_PRIORITY (configMAX_PRIORITIES - 1) ::: #define ULTRAWIDELOCK_DW3000_ISR_TASK_PRIORITY (configMAX_PRIORITIES - 4)"
	...
	"the reset line is driven high instead of released ::: 	nrf_gpio_cfg_input(ULTRAWIDELOCK_DW3000_PIN_RST, NRF_GPIO_PIN_NOPULL);

	/* Long enough ::: 	nrf_gpio_pin_set(ULTRAWIDELOCK_DW3000_PIN_RST);
...
void dw3000_hw_interrupt_disable"
	"shutting down leaves the GPIOTE channel live ::: nrf_gpiote_event_disable(NRF_GPIOTE, ULTRAWIDELOCK_DW3000_GPIOTE_CHANNEL); ::: (void)0;"
	"shutting down leaves the SPI bus up ::: 	nrf_gpiote_event_disable(NRF_GPIOTE, ULTRAWIDELOCK_DW3000_GPIOTE_CHANNEL);
	dw3000_spi_fini(); ::: 	nrf_gpiote_event_disable(NRF_GPIOTE, ULTRAWIDELOCK_DW3000_GPIOTE_CHANNEL);"
)
