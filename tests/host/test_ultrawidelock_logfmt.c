/**
 * @file test_ultrawidelock_logfmt.c — the PRETTY console formatter + quiet-list on host.
 *
 * Drives ultrawidelock_logfmt.c / ultrawidelock_logquiet.c through the fake Zephyr logging surface
...
extern int (*const logfake_sys_init_ultrawidelock_logfmt_init)(void);
extern int (*const logfake_sys_init_ultrawidelock_logquiet_init)(void);
...
{
	/* --- ultrawidelock_logquiet: mute known sources, skip absent ones ------------- */
	t_group("logquiet");
	logfake_reset();
	logfake.source_name[0] = "chip";
	logfake.source_name[1] = "access_document"; /* "bt_adv" absent */
	T_EQ("quiet init rc", logfake_sys_init_ultrawidelock_logquiet_init(), 0);
	...
	T_EQ("init rc", logfake_sys_init_ultrawidelock_logfmt_init(), 0);
	...
	(void)logfake_sys_init_ultrawidelock_logfmt_init();
