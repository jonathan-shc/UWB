...
#include <ultrawidelock/uwb.h>  /* ultrawidelock_uwb_start_cred / ultrawidelock_uwb_stop */
...
bool fr_replay_run(const uint8_t *trace, size_t len, struct fr_replay_result *out)
{
	...
	struct ultrawidelock_uwb_cred_cfg cfg;
	...
	while ((type = fr_read_next(&r, &rec)) > 0) {
		if (type == FR_REC_CONFIG) {
			...
			if (ultrawidelock_uwb_start_cred(&cfg) != 0) {
				out->err_at = idx;
				return false;
			}
			have_config = true;
