/**
 * @file test_woz_fusion.c — woz_fusion suite: side-of-door from the signed
...
static void side_of_door(void)
{
	...
	v = woz_fusion_eval(&k_cfg, 800, 1400);
	T_OK("side.inside.ok", v.geometry_ok);
	T_EQ("side.inside", v.side, WOZ_SIDE_INSIDE);
	T_EQ("side.inside.delta", v.delta_mm, -600);

	/* Clearly nearer the outside anchor. */
	v = woz_fusion_eval(&k_cfg, 1400, 800);
	T_OK("side.outside.ok", v.geometry_ok);
	T_EQ("side.outside", v.side, WOZ_SIDE_OUTSIDE);
...
static void deadband_is_honest(void)
{
	...
	v = woz_fusion_eval(&k_cfg, 1000, 1000);
	T_OK("dead.centre.ok", v.geometry_ok);
	T_EQ("dead.centre", v.side, WOZ_SIDE_UNKNOWN);

	/* Just inside the band: still UNKNOWN rather than whichever way noise fell. */
	v = woz_fusion_eval(&k_cfg, 1030, 1000);
	T_EQ("dead.inside_band", v.side, WOZ_SIDE_UNKNOWN);

	/* Just outside it: now the sign is trusted. */
	v = woz_fusion_eval(&k_cfg, 1061, 1000);
	T_EQ("dead.outside_band", v.side, WOZ_SIDE_OUTSIDE);
}
...
static void triangle_gate_refuses_impossible_pairs(void)
{
	 ...
	 */
	v = woz_fusion_eval(&k_cfg, 200, 300);
	T_OK("tri.too_short.rejected", !v.geometry_ok);
	T_EQ("tri.too_short.side", v.side, WOZ_SIDE_UNKNOWN);
	 ...
	 * different targets.
	 */
	v = woz_fusion_eval(&k_cfg, 400, 2000);
	T_OK("tri.too_unequal.rejected", !v.geometry_ok);
	T_EQ("tri.too_unequal.side", v.side, WOZ_SIDE_UNKNOWN);
	...
	v = woz_fusion_eval(&k_cfg, 600, 600);
	T_OK("tri.collinear_ok", v.geometry_ok);
	v = woz_fusion_eval(&k_cfg, 100, 1300);
	...
	v = woz_fusion_eval(&k_cfg, 570, 570); /* sum 1140, baseline - tol = 1110 */
	T_OK("tri.within_tol", v.geometry_ok);
	v = woz_fusion_eval(&k_cfg, 540, 540); /* sum 1080, below 1110 */
 ...
 *
 * woz_fusion.h states plainly that a SYMMETRIC relay -- one inflating both
...
static void predict_gate(void)
{
	...
	v = woz_fusion_eval(&k_cfg, 800, 1400); /* inside */
	T_OK("pred.inside_allows", woz_fusion_may_predict(&v));

	v = woz_fusion_eval(&k_cfg, 1400, 800); /* outside */
	T_OK("pred.outside_withholds", !woz_fusion_may_predict(&v));
	...
	v = woz_fusion_eval(&k_cfg, 1000, 1000);
	T_EQ("pred.unknown_is_unknown", v.side, WOZ_SIDE_UNKNOWN);
	T_OK("pred.unknown_allows", woz_fusion_may_predict(&v));

	/* No verdict at all is the same as no satellite. */
	T_OK("pred.null_allows", woz_fusion_may_predict(NULL));

	/* But a pair that failed the triangle test is real evidence of a fault
	 * or an attack, and that does withhold. */
	v = woz_fusion_eval(&k_cfg, 200, 300);
	T_OK("pred.bad_geometry_withholds", !woz_fusion_may_predict(&v));
}

static void rejects_bad_input(void)
{
	struct woz_fusion_verdict v;
	struct woz_fusion_cfg zero = {0, 0, 0};

	t_group("input: nonsense in, UNKNOWN out");

	v = woz_fusion_eval(NULL, 800, 1400);
	T_OK("in.null_cfg", !v.geometry_ok);
	v = woz_fusion_eval(&zero, 800, 1400);
	T_OK("in.zero_baseline", !v.geometry_ok);
	v = woz_fusion_eval(&k_cfg, -1, 1400);
