/**
 * @file test_woz_report.c — woz_report suite: the C2 line codec.
 *
...
static struct woz_range_report sample(void)
{
	...
	r.quality = 87u;
	r.trust = 5u;
	r.flags = WOZ_REPORT_F_VALID | WOZ_REPORT_F_CALIBRATED;
	r.dropped = 3u;
	r.accepted = 991u;
	return r;
}
...
static void crc_matches_the_standard(void)
{
	...
	T_EQ("crc.check_value", woz_report_crc16("123456789", 9u), 0x29B1u);
}
...
static void extreme_values(void)
{
	...
	n = woz_report_format(&r, buf, sizeof(buf));
	T_OK("ext.max.fits", n > 0 && (size_t)n <= WOZ_REPORT_LINE_MAX);
	T_EQ("ext.max.parses", woz_report_parse(buf, (size_t)n, &back), 0);
	...
	T_EQ("ext.min.parses", woz_report_parse(buf, (size_t)n, &back), 0);
	T_EQ("ext.min.value", back.d_mm, -2147483647 - 1);

	/* Near-contact rounds go slightly negative; that is a real measurement. */
	r.d_mm = -47;
	n = woz_report_format(&r, buf, sizeof(buf));
	T_EQ("ext.neg.parses", woz_report_parse(buf, (size_t)n, &back), 0);
	...
	T_EQ("ext.zero.parses", woz_report_parse(buf, (size_t)n, &back), 0);
...
static void single_flipped_digit_is_caught(void)
{
	...
	for (i = 0; i < n; i++) {
		...
		buf[i] = (char)(save + 1);
		mutated++;
		if (woz_report_parse(buf, (size_t)n, &back) != 0) {
			caught++;
		}
		buf[i] = save;
	}
...
static void foreign_version_is_refused(void)
{
	...
	n = woz_report_format(&r, buf, sizeof(buf));
	buf[3] = '2'; /* ARP1 -> ARP2 */
	T_EQ("ver.arp2_refused", woz_report_parse(buf, (size_t)n, &back), -EBADMSG);

	buf[0] = 'X';
	buf[3] = '1';
	T_EQ("ver.junk_refused", woz_report_parse(buf, (size_t)n, &back), -EBADMSG);

	/* Console noise must not parse as a report. */
	{
		const char *noise = "[00:00:01.234] <inf> main: ANCHOR rounds=100\n";

		T_EQ("ver.log_line", woz_report_parse(noise, strlen(noise), &back), -EBADMSG);
	}
}
...
static void truncation_and_junk(void)
{
	...
	for (i = 1; i < n - 1; i++) {
		...
		if (woz_report_parse(cut, (size_t)i, &back) == 0) {
			accepted++;
		}
	}
	 ...
	 * is still refused: the parser counts fields, not just checksum. */
	{
		...
		T_EQ("trunc.too_few_fields",
		     woz_report_parse(short_line, strlen(short_line), &back), -EBADMSG);
	}
	...
	/* Extra trailing junk inside the covered range. */
	{
		...
		T_EQ("trunc.trailing_junk", woz_report_parse(tail, strlen(tail), &back), -EBADMSG);
	}
}
...
static void line_endings(void)
{
	...
	crlf[n - 1] = '\r';
	crlf[n] = '\n';
	T_EQ("eol.crlf", woz_report_parse(crlf, (size_t)n + 1, &back), 0);
	T_OK("eol.crlf_same", same(&r, &back));

	/* No terminator at all. */
	T_EQ("eol.none", woz_report_parse(buf, (size_t)n - 1, &back), 0);
...
static void small_buffer_never_writes_a_partial_line(void)
{
	...
	full = woz_report_format(&r, buf, sizeof(buf));
	...
	T_EQ("cap.exact_fits", woz_report_format(&r, buf, (size_t)full), full);
}
...
static void rejects_bad_input(void)
{
	...
	T_EQ("in.null_report", woz_report_format(NULL, buf, sizeof(buf)), -EINVAL);
	T_EQ("in.null_buf", woz_report_format(&r, NULL, sizeof(buf)), -EINVAL);
	T_EQ("in.null_line", woz_report_parse(NULL, 10u, &back), -EINVAL);
	T_EQ("in.null_out", woz_report_parse("ARP1", 4u, NULL), -EINVAL);
	T_EQ("in.empty", woz_report_parse("", 0u, &back), -EBADMSG);

	/* The CRC helper is used by the host-side consumer, so its degenerate
	 * cases are part of the contract. */
	T_EQ("in.crc_null", woz_report_crc16(NULL, 4u), 0u);
	T_EQ("in.crc_empty", woz_report_crc16("", 0u), 0xFFFFu);
}
