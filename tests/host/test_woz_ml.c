/**
 * @file test_woz_ml.c — certify the shipped LOS/NLOS tree against sklearn.
 *
...
#include "data/woz_ml_los_lin_vectors.h"
#include "data/woz_ml_los_vectors.h"

void test_woz_ml(void)
{
	t_group("woz_ml: tree vs sklearn");
	...
	T_EQ("feature count matches the vectors",
	     WOZ_ML_LOS_VEC_FEATURES, WOZ_ML_LOS_N_FEATURES);

	int mismatch = 0;
	int leaves_clear = 0, leaves_obstructed = 0;

	for (int i = 0; i < WOZ_ML_LOS_VEC_COUNT; i++) {
		const enum woz_ml_los_class got =
			woz_ml_los_classify(woz_ml_los_vec_in[i]);

		if ((int)got != (int)woz_ml_los_vec_expect[i]) {
			mismatch++;
		}
		if (got == WOZ_ML_LOS_CLEAR) {
			leaves_clear++;
		} else {
			leaves_obstructed++;
		}
	...
	t_group("woz_ml: vendor rule and disagreement");
	...
	T_EQ("5.9 dB is clear", woz_ml_los_vendor(5.9f), WOZ_ML_LOS_CLEAR);
	T_EQ("exactly 6 dB is clear", woz_ml_los_vendor(6.0f), WOZ_ML_LOS_CLEAR);
	T_EQ("6.1 dB is obstructed", woz_ml_los_vendor(6.1f), WOZ_ML_LOS_OBSTRUCTED);
	...
	for (int i = 0; i < WOZ_ML_LOS_VEC_COUNT; i++) {
		...
		if (woz_ml_los_disagrees(woz_ml_los_vec_in[i], (i % 2) ? 9.0f : 3.0f)) {
			disagree++;
		}
	}

	T_OK("tree and vendor rule disagree sometimes", disagree > 0);
	T_OK("but not always", disagree < WOZ_ML_LOS_VEC_COUNT);
	...
	T_OK("range correction is 0 dB at 1 m",
	     fabsf(woz_ml_los_range_correction(100) - 0.0f) < 0.2f);
	T_OK("range correction is +6.02 dB at 2 m",
	     fabsf(woz_ml_los_range_correction(200) - 6.0206f) < 0.2f);
	T_OK("range correction is -20 dB at 10 cm",
	     fabsf(woz_ml_los_range_correction(10) + 20.0f) < 0.2f);
	T_OK("range correction clamps below 10 cm rather than diverging",
	     woz_ml_los_range_correction(0) == woz_ml_los_range_correction(10));
	T_OK("range correction is monotonic across the reader's whole span", ({
		     bool up = true;
		     for (uint16_t d = 11; d < 2000; d++) {
			     if (woz_ml_los_range_correction(d) <
				 woz_ml_los_range_correction((uint16_t)(d - 1))) {
				     up = false;
				     break;
			     }
		     }
	...
	T_OK("fp_resid is fp_pwr plus the correction",
	     fabsf(woz_ml_los_fp_resid(-80.0f, 200) - (-80.0f + 6.0206f)) < 0.2f);
	...
	float extreme[WOZ_ML_LOS_N_FEATURES];

	for (int i = 0; i < WOZ_ML_LOS_N_FEATURES; i++) {
		extreme[i] = 1e30f;
	}
	T_OK("huge features saturate to a valid class",
	     woz_ml_los_classify(extreme) == WOZ_ML_LOS_CLEAR ||
	     woz_ml_los_classify(extreme) == WOZ_ML_LOS_OBSTRUCTED);
	...
	T_OK("log2(1) is 0", fabsf(woz_ml_log2_u64(1) - 0.0f) < 1e-6f);
	T_OK("log2(2) is 1", fabsf(woz_ml_log2_u64(2) - 1.0f) < 1e-6f);
	T_OK("log2(1024) is 10", fabsf(woz_ml_log2_u64(1024) - 10.0f) < 1e-6f);
	...
	T_OK("log2(2^40) is 40", fabsf(woz_ml_log2_u64(1ull << 40) - 40.0f) < 1e-6f);
	T_OK("log2(2^63) is 63", fabsf(woz_ml_log2_u64(1ull << 63) - 63.0f) < 1e-6f);
	 ...
	 * still there. */
	T_OK("log2(0) is 0 rather than undefined", woz_ml_log2_u64(0) == 0.0f);

	T_OK("log2 is monotonic over four decades", ({
		     bool up = true;
		     for (uint64_t v = 2; v < 100000; v = v + 1 + v / 64) {
			     if (woz_ml_log2_u64(v) <= woz_ml_log2_u64(v - 1)) {
				     up = false;
				     break;
			     }
		     }
	...
	struct woz_ml_cia cia = {
		.f1 = 11000, .f2 = 9000, .f3 = 7000, .accum_count = 48, .channel_area = 13,
	};
	float feat[WOZ_ML_LOS_N_FEATURES];
	float pwr_diff = 0.0f;

	T_OK("a good reception is accepted", woz_ml_los_features(&cia, 100, feat, &pwr_diff));
	T_OK("rx_pwr matches the float64 formula",
	     fabsf(feat[WOZ_ML_LOS_F_RX_PWR] - (-93.0503f)) < 0.01f);
	...
	T_OK("fp_resid equals fp_pwr at 1 m",
	     fabsf(feat[WOZ_ML_LOS_F_FP_RESID] - (-71.3681f)) < 0.01f);
	...
	float feat_2m[WOZ_ML_LOS_N_FEATURES];

	T_OK("2 m is accepted too", woz_ml_los_features(&cia, 200, feat_2m, NULL));
	T_OK("fp_resid at 2 m is 6.02 dB above 1 m",
	     fabsf((feat_2m[WOZ_ML_LOS_F_FP_RESID] - feat[WOZ_ML_LOS_F_FP_RESID]) - 6.0206f)
		     < 0.01f);
	T_OK("rx_pwr does not move with range",
	     feat_2m[WOZ_ML_LOS_F_RX_PWR] == feat[WOZ_ML_LOS_F_RX_PWR]);
	...
	T_OK("pwr_diff may be NULL", woz_ml_los_features(&cia, 100, feat, NULL));
	...
	struct woz_ml_cia bad = cia;

	bad.accum_count = 0;
	T_OK("zero accumulator count is rejected", !woz_ml_los_features(&bad, 100, feat, NULL));

	bad = cia;
	bad.channel_area = 0;
	T_OK("zero channel area is rejected", !woz_ml_los_features(&bad, 100, feat, NULL));

	bad = cia;
	bad.f1 = bad.f2 = bad.f3 = 0;
	T_OK("all-zero F1..F3 is rejected", !woz_ml_los_features(&bad, 100, feat, NULL));
	...
	T_OK("a single zero among F1..F3 is still a valid reception",
	     woz_ml_los_features(&bad, 100, feat, NULL));
	...
	T_OK("extracted features classify to a valid class", ({
		     woz_ml_los_features(&cia, 150, feat, &pwr_diff);
		     const enum woz_ml_los_class c = woz_ml_los_classify(feat);
		     c == WOZ_ML_LOS_CLEAR || c == WOZ_ML_LOS_OBSTRUCTED;
	     }));

	t_group("woz_ml: confidence");
	...
	for (size_t i = 0; i < sizeof(woz_ml_los_lin_vectors) /
				 sizeof(woz_ml_los_lin_vectors[0]); i++) {
		const float got = woz_ml_los_confidence(woz_ml_los_lin_vectors[i].feat);

		if (fabsf(got - woz_ml_los_lin_vectors[i].confidence) > 1e-3f) {
			conf_mismatch++;
		}
		if (got < 0.0f) {
			negative++;
	 ...
	 */
	float prev = woz_ml_los_confidence((const float[]){-40.0f, -93.0f});
	int falls = 0, rises = 0;

	for (float fp = -42.0f; fp >= -160.0f; fp -= 2.0f) {
		const float now = woz_ml_los_confidence((const float[]){fp, -93.0f});
	...
	float lowest = woz_ml_los_confidence((const float[]){-40.0f, -93.0f});
	...
	t_group("woz_ml: range correction");
	 ...
	 */
	T_EQ("100 cm clear: 75 -> 101", woz_ml_los_range_true_cm(75, WOZ_ML_LOS_CLEAR), 101);
	T_EQ("200 cm clear: 174 -> 200", woz_ml_los_range_true_cm(174, WOZ_ML_LOS_CLEAR), 200);
	T_EQ("100 cm blocked: 155 -> 96",
	     woz_ml_los_range_true_cm(155, WOZ_ML_LOS_OBSTRUCTED), 96);
	T_EQ("200 cm blocked: 263 -> 204",
	     woz_ml_los_range_true_cm(263, WOZ_ML_LOS_OBSTRUCTED), 204);
	...
	T_EQ("clear correction is range-independent",
	     woz_ml_los_range_true_cm(500, WOZ_ML_LOS_CLEAR) -
		     woz_ml_los_range_true_cm(100, WOZ_ML_LOS_CLEAR),
	     400);
	...
	T_OK("obstructed corrects downward relative to clear",
	     woz_ml_los_range_true_cm(200, WOZ_ML_LOS_OBSTRUCTED) <
		     woz_ml_los_range_true_cm(200, WOZ_ML_LOS_CLEAR));
	...
	T_EQ("an obstructed reading below the bias clamps at 0",
	     woz_ml_los_range_true_cm(10, WOZ_ML_LOS_OBSTRUCTED), 0);
	T_EQ("and exactly at the bias it is 0",
	     woz_ml_los_range_true_cm(59, WOZ_ML_LOS_OBSTRUCTED), 0);
	T_OK("just above the bias it is positive",
	     woz_ml_los_range_true_cm(61, WOZ_ML_LOS_OBSTRUCTED) > 0);

	/* The largest range a reader reports must not overflow the uint16 return. */
	T_OK("the top of the reader's span does not wrap",
	     woz_ml_los_range_true_cm(65000, WOZ_ML_LOS_CLEAR) > 65000);
}
