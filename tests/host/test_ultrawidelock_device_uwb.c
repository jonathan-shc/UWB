/** @file test_ultrawidelock_device_uwb.c — device-side M1-M4 setup codec, looped back
 *  against the real reader session. The device parses the reader's M1/M3 and
 *  builds M2/M4; the reader session (ultrawidelock_uwb_msg.c) must accept those and walk
...
#include "ultrawidelock_uwb_internal.h" /* session struct + states + create/handle/destroy */
#include "ultrawidelock_uwb_msg.h"      /* header accessors + free */
#include "ultrawidelock_uwb_msg_builder.h"
#include "ultrawidelock_uwb_msg_parser.h"
#include "ultrawidelock_uwb_msg_spec.h"
#include "test.h"

/* transmit + event capture (same shape as test_ultrawidelock_uwb_session.c) */
static struct {
...
void test_ultrawidelock_device_uwb(void)
{
	...
	cfg.preferred_hopping_configs.configs[0] = ULTRAWIDELOCK_HOPPING_CONFIG_CONTINUOUS_DEFAULT;
	...
	T_EQ("m3.emitted", ultrawidelock_uwb_msg_message_id(g_tx.msg->data),
	...
	m2 = ultrawidelock_dev_uwb_build_m2(&m2p);
	{
		...
		while ((a = ultrawidelock_uwb_msg_next_attribute(&p)) != NULL) {
			if (a->id == ULTRAWIDELOCK_UWB_RANGING_SERVICE_ATTR_CONFIGURATION_IDENTIFIER) {
				ultrawidelock_uwb_msg_read_u16(a, "cid", &cid);
			} else if (a->id == ULTRAWIDELOCK_UWB_RANGING_SERVICE_ATTR_CHANNEL_BITMASK) {
				ultrawidelock_uwb_msg_read_u8(a, "ch", &ch);
			} else if (a->id == ULTRAWIDELOCK_UWB_RANGING_SERVICE_ATTR_RAN_MULTIPLIER) {
				ultrawidelock_uwb_msg_read_u8(a, "ran", &ran);
			}
		}
