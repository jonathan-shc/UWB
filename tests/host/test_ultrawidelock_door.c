/**
 * @file test_ultrawidelock_door.c — ultrawidelock_door suite: the law-of-cosines angle mapping and
...
static const struct ultrawidelock_door_cfg k_cfg = {
...
static int32_t d_for_deg(const struct ultrawidelock_door_cfg *c, double deg)
{
...
static void angle_roundtrip(void)
{
	...
	for (int deg = 0; deg <= 120; deg += 5) {
		...
		int32_t got = ultrawidelock_door_angle_mddeg(&k_cfg, d);
		int32_t want = deg * 1000;
		int32_t tol = (deg <= 90) ? 700 : 2000;
		char nm[48];

		snprintf(nm, sizeof(nm), "angle.%ddeg", deg);
		T_OK(nm, got != ULTRAWIDELOCK_DOOR_ANGLE_INVALID && labs(got - want) <= tol);
	}
}
...
static void angle_rejects_impossible(void)
{
	...
	T_EQ("angle.too_far", ultrawidelock_door_angle_mddeg(&k_cfg, 2500),
	     ULTRAWIDELOCK_DOOR_ANGLE_INVALID);
	T_EQ("angle.negative", ultrawidelock_door_angle_mddeg(&k_cfg, -1),
	     ULTRAWIDELOCK_DOOR_ANGLE_INVALID);
	T_EQ("angle.null_cfg", ultrawidelock_door_angle_mddeg(NULL, 500),
	     ULTRAWIDELOCK_DOOR_ANGLE_INVALID);

	/* Exactly at the limits is legal, and saturates rather than failing. */
	T_EQ("angle.shut", ultrawidelock_door_angle_mddeg(&k_cfg, 0), 0);
	T_OK("angle.flat", ultrawidelock_door_angle_mddeg(&k_cfg, 2 * DOOR_MM) >= 179000);
}

/*
 * THE MOUNTING REQUIREMENT, asserted rather than left in a comment.
 *
 * ultrawidelock_door.h claims a and b must be roughly equal or the closed-versus-ajar
...
{
	struct ultrawidelock_door_cfg lop = {
	...
	good_shut = ultrawidelock_door_resolution_mddeg(&k_cfg, 1000, 30);
	bad_shut = ultrawidelock_door_resolution_mddeg(&lop, 1000, 30);
	good_open = ultrawidelock_door_resolution_mddeg(&k_cfg, 35000, 30);
	...
	T_EQ("res.flat_is_blind", ultrawidelock_door_resolution_mddeg(&k_cfg, 180000, 30), INT32_MAX);
	T_EQ("res.null", ultrawidelock_door_resolution_mddeg(NULL, 0, 30), INT32_MAX);
	T_EQ("res.zero_jitter", ultrawidelock_door_resolution_mddeg(&k_cfg, 1000, 0), INT32_MAX);
}

/** Feed one angle n times; returns the state after. */
static enum ultrawidelock_door_state feed_deg(struct ultrawidelock_door *d, double deg, int n)
{
	enum ultrawidelock_door_state s = ULTRAWIDELOCK_DOOR_UNKNOWN;

	for (int i = 0; i < n; i++) {
		s = ultrawidelock_door_feed(d, d_for_deg(&d->cfg, deg));
	}
	return s;
}
...
{
	struct ultrawidelock_door d;

	t_group("state machine: dwell and hysteresis");

	T_OK("sm.init", ultrawidelock_door_init(&d, &k_cfg, NULL));
	T_EQ("sm.starts_unknown", d.state, ULTRAWIDELOCK_DOOR_UNKNOWN);

	/* Dwell is 3: two samples are not enough to commit, the third is. */
	T_EQ("sm.one_sample_holds", feed_deg(&d, 0.0, 1), ULTRAWIDELOCK_DOOR_UNKNOWN);
	T_EQ("sm.two_samples_hold", feed_deg(&d, 0.0, 1), ULTRAWIDELOCK_DOOR_UNKNOWN);
	T_EQ("sm.third_commits", feed_deg(&d, 0.0, 1), ULTRAWIDELOCK_DOOR_CLOSED);

	/* Hysteresis: 3 degrees is past closed_enter (2) but not past
	 * closed_leave (5), so a closed door stays closed. */
	T_EQ("sm.hysteresis_holds_closed", feed_deg(&d, 3.0, 5), ULTRAWIDELOCK_DOOR_CLOSED);

	/* Past closed_leave it becomes ajar, after the dwell. */
	T_EQ("sm.opens_to_ajar", feed_deg(&d, 15.0, 3), ULTRAWIDELOCK_DOOR_AJAR);

	/* Past open_enter (35) it becomes open. */
	T_EQ("sm.reaches_open", feed_deg(&d, 45.0, 3), ULTRAWIDELOCK_DOOR_OPEN);
	...
	T_EQ("sm.hysteresis_holds_open", feed_deg(&d, 32.0, 5), ULTRAWIDELOCK_DOOR_OPEN);
	T_EQ("sm.closes_to_ajar", feed_deg(&d, 20.0, 3), ULTRAWIDELOCK_DOOR_AJAR);
	T_EQ("sm.back_to_closed", feed_deg(&d, 0.0, 3), ULTRAWIDELOCK_DOOR_CLOSED);
}
...
static void state_survives_a_bad_read(void)
{
	...
	T_OK("bad.init", ultrawidelock_door_init(&d, &k_cfg, NULL));
	T_EQ("bad.closed", feed_deg(&d, 0.0, 3), ULTRAWIDELOCK_DOOR_CLOSED);
	...
	T_EQ("bad.holds_state", ultrawidelock_door_feed(&d, 9999), ULTRAWIDELOCK_DOOR_CLOSED);
	T_EQ("bad.angle_invalid", d.last_mddeg, ULTRAWIDELOCK_DOOR_ANGLE_INVALID);

	/* And it breaks the run, so a half-finished transition has to start over. */
	T_EQ("bad.still_closed", feed_deg(&d, 45.0, 2), ULTRAWIDELOCK_DOOR_CLOSED);
	T_EQ("bad.then_commits", feed_deg(&d, 45.0, 1), ULTRAWIDELOCK_DOOR_OPEN);
}

static void rejects_bad_config(void)
{
	struct ultrawidelock_door d;
	struct ultrawidelock_door_cfg bad = {.hinge_to_frame_mm = 0, .hinge_to_leaf_mm = 850};

	t_group("config: refuses geometry it cannot use");
	T_OK("cfg.zero_arm", !ultrawidelock_door_init(&d, &bad, NULL));
	T_OK("cfg.null_cfg", !ultrawidelock_door_init(&d, NULL, NULL));
	T_OK("cfg.null_self", !ultrawidelock_door_init(NULL, &k_cfg, NULL));
}
