...
void test_ultrawidelock_nfc(void)
{
	...
	uint8_t command[ULTRAWIDELOCK_CRED_SELECT_COMMAND_SIZE];
	T_EQ("build expedited SELECT",
	     ultrawidelock_cred_build_select_command(ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, command),
	...
	T_EQ("parse SELECT example",
	     ultrawidelock_cred_parse_select_response(
		     response, sizeof(response), ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &version),
	     ULTRAWIDELOCK_CRED_SELECT_OK);
	...
	T_EQ("reject unsuccessful status",
	     ultrawidelock_cred_parse_select_response(malformed, sizeof(malformed),
					     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &version),
	     ULTRAWIDELOCK_CRED_SELECT_STATUS_ERROR);
	...
	T_EQ("reject wrong selected AID",
	     ultrawidelock_cred_parse_select_response(malformed, sizeof(malformed),
					     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &version),
	     ULTRAWIDELOCK_CRED_SELECT_WRONG_APPLICATION);
	...
	T_EQ("reject unsupported version",
	     ultrawidelock_cred_parse_select_response(malformed, sizeof(malformed),
					     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &version),
	     ULTRAWIDELOCK_CRED_SELECT_VERSION_NOT_SUPPORTED);

	static const uint8_t long_form[] = { 0xb3, 0x81, 0x80 };
	struct ultrawidelock_cred_tlv tlv;
	size_t offset = 0;
	T_EQ("reject truncated long TLV",
	     ultrawidelock_cred_tlv_next(long_form, sizeof(long_form), &offset, &tlv),
	     ULTRAWIDELOCK_CRED_TLV_INVALID);
	static const uint8_t non_minimal[] = { 0x80, 0x81, 0x01, 0x00 };
	offset = 0;
	T_EQ("reject non-minimal DER length",
	     ultrawidelock_cred_tlv_next(non_minimal, sizeof(non_minimal), &offset, &tlv),
	     ULTRAWIDELOCK_CRED_TLV_INVALID);

	/* Section 14.3 published expedited-standard AUTH0 transcript. */
	uint8_t reader_ephemeral[ULTRAWIDELOCK_CRED_PUBLIC_KEY_SIZE];
	uint8_t transaction_id[ULTRAWIDELOCK_CRED_TRANSACTION_ID_SIZE];
	uint8_t reader_id[ULTRAWIDELOCK_CRED_READER_ID_SIZE];
	...
	struct ultrawidelock_cred_auth0_command auth0 = {
	...
	T_EQ("build AUTH0", ultrawidelock_cred_build_auth0_command(&auth0, auth0_command,
							 sizeof(auth0_command), &auth0_command_length),
	     ULTRAWIDELOCK_CRED_AUTH_OK);
	uint8_t expected_auth0[ULTRAWIDELOCK_CRED_AUTH0_STANDARD_COMMAND_SIZE];
	...
	struct ultrawidelock_cred_auth0_response auth0_response;
	T_EQ("parse AUTH0 standard response",
	     ultrawidelock_cred_parse_auth0_response(auth0_response_bytes, auth0_response_length, 0,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_OK);
	...
	T_EQ("reject missing fast cryptogram",
	     ultrawidelock_cred_parse_auth0_response(auth0_response_bytes, auth0_response_length, 1,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_WRONG_CONTENT);
	...
	T_EQ("parse AUTH0 fast response",
	     ultrawidelock_cred_parse_auth0_response(auth0_response_bytes, fast_response_length, 1,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_OK);
	T_EQ("AUTH0 fast cryptogram length", auth0_response.cryptogram_length, 64);
	T_EQ("reject unexpected fast cryptogram",
	     ultrawidelock_cred_parse_auth0_response(auth0_response_bytes, fast_response_length, 0,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_WRONG_CONTENT);
	...
	T_EQ("restore AUTH0 standard response",
	     ultrawidelock_cred_parse_auth0_response(auth0_response_bytes, restored_standard_length, 0,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_OK);

	uint8_t authentication_data[ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE];
	T_EQ("build AUTH1 authentication data",
	     ultrawidelock_cred_build_authentication_data(reader_id,
					       auth0_response.credential_ephemeral_public_key,
					       reader_ephemeral, transaction_id, 0x415d9569,
					       authentication_data), ULTRAWIDELOCK_CRED_AUTH_OK);
	uint8_t expected_authentication_data[ULTRAWIDELOCK_CRED_AUTH_DATA_SIZE];
	...
	uint8_t auth1_command[ULTRAWIDELOCK_CRED_AUTH1_COMMAND_SIZE];
	size_t auth1_command_length = 0;
	T_EQ("build AUTH1 command", ultrawidelock_cred_build_auth1_command(1, signature, auth1_command,
							       sizeof(auth1_command),
							       &auth1_command_length),
	     ULTRAWIDELOCK_CRED_AUTH_OK);
	uint8_t expected_auth1[ULTRAWIDELOCK_CRED_AUTH1_COMMAND_SIZE];
	...
	struct ultrawidelock_cred_auth1_response auth1_response;
	T_EQ("parse AUTH1 plaintext",
	     ultrawidelock_cred_parse_auth1_plaintext(auth1_plaintext, auth1_plaintext_length, 1,
					     &auth1_response), ULTRAWIDELOCK_CRED_AUTH_OK);
	...
	uint8_t step_up_select[ULTRAWIDELOCK_CRED_SELECT_COMMAND_SIZE];
	T_EQ("build step-up SELECT",
	     ultrawidelock_cred_build_select_command(ULTRAWIDELOCK_CRED_SELECT_STEP_UP, step_up_select),
	     ULTRAWIDELOCK_CRED_SELECT_OK);
	T_OK("step-up SELECT AID", memcmp(step_up_select + 5, ultrawidelock_cred_step_up_aid,
					  ULTRAWIDELOCK_CRED_AID_SIZE) == 0);
	T_EQ("reject unknown SELECT phase",
	     ultrawidelock_cred_build_select_command((enum ultrawidelock_cred_select_phase)0,
						     step_up_select),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_ARGUMENT);

	struct ultrawidelock_cred_select_response select_result;
	T_EQ("reject null proprietary TLV",
	     ultrawidelock_cred_parse_proprietary_information(NULL, 0, ULTRAWIDELOCK_CRED_SELECT_EXPEDITED,
						     &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_ARGUMENT);
	static const uint8_t not_a5[] = { 0x80, 0x01, 0xaa };
	T_EQ("reject non-A5 proprietary TLV",
	     ultrawidelock_cred_parse_proprietary_information(not_a5, sizeof(not_a5),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	static const uint8_t inner_truncated[] = { 0xa5, 0x01, 0x80 };
	T_EQ("reject truncated proprietary item",
	     ultrawidelock_cred_parse_proprietary_information(inner_truncated, sizeof(inner_truncated),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	static const uint8_t wrong_type[] = { 0xa5, 0x04, 0x80, 0x02, 0x00, 0x01 };
	T_EQ("reject wrong application type",
	     ultrawidelock_cred_parse_proprietary_information(wrong_type, sizeof(wrong_type),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_WRONG_TYPE);
	static const uint8_t duplicate_type[] = { 0xa5, 0x08, 0x80, 0x02, 0x00, 0x00,
						  0x80, 0x02, 0x00, 0x00 };
	T_EQ("reject duplicate application type",
	     ultrawidelock_cred_parse_proprietary_information(duplicate_type, sizeof(duplicate_type),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_WRONG_TYPE);
	static const uint8_t missing_type[] = { 0xa5, 0x04, 0x5c, 0x02, 0x01, 0x00 };
	T_EQ("reject missing application type",
	     ultrawidelock_cred_parse_proprietary_information(missing_type, sizeof(missing_type),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_WRONG_TYPE);
	static const uint8_t odd_versions[] = { 0xa5, 0x07, 0x80, 0x02, 0x00, 0x00,
						0x5c, 0x01, 0x01 };
	T_EQ("reject odd version list",
	     ultrawidelock_cred_parse_proprietary_information(odd_versions, sizeof(odd_versions),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	...
	T_EQ("reject sub-minimum extended limits",
	     ultrawidelock_cred_parse_proprietary_information(small_limits, sizeof(small_limits),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	...
	T_EQ("reject non-integer extended limit",
	     ultrawidelock_cred_parse_proprietary_information(wrong_size_tag, sizeof(wrong_size_tag),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	...
	T_EQ("reject trailing extended-limit data",
	     ultrawidelock_cred_parse_proprietary_information(trailing_size, sizeof(trailing_size),
						     ULTRAWIDELOCK_CRED_SELECT_EXPEDITED, &select_result),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	...
	T_EQ("reject duplicate extended limits",
	     ultrawidelock_cred_parse_proprietary_information(duplicate_extended,
	...
	T_EQ("parse step-up SELECT response",
	     ultrawidelock_cred_parse_select_response(step_up_response, sizeof(step_up_response),
					     ULTRAWIDELOCK_CRED_SELECT_STEP_UP, &step_up_version),
	     ULTRAWIDELOCK_CRED_SELECT_OK);
	T_EQ("reject null version out",
	     ultrawidelock_cred_parse_select_response(step_up_response, sizeof(step_up_response),
					     ULTRAWIDELOCK_CRED_SELECT_STEP_UP, NULL),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_ARGUMENT);
	T_EQ("reject short SELECT response",
	     ultrawidelock_cred_parse_select_response(
		     step_up_response, 3, ULTRAWIDELOCK_CRED_SELECT_STEP_UP, &step_up_version),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	static const uint8_t not_fci[] = { 0x70, 0x02, 0x00, 0x00, 0x90, 0x00 };
	T_EQ("reject non-FCI response",
	     ultrawidelock_cred_parse_select_response(not_fci, sizeof(not_fci),
					     ULTRAWIDELOCK_CRED_SELECT_STEP_UP, &step_up_version),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	...
	T_EQ("reject FCI without proprietary",
	     ultrawidelock_cred_parse_select_response(missing_proprietary, sizeof(missing_proprietary),
					     ULTRAWIDELOCK_CRED_SELECT_STEP_UP, &step_up_version),
	     ULTRAWIDELOCK_CRED_SELECT_INVALID_APDU);
	...
	T_EQ("AUTH0 rejects null params",
	     ultrawidelock_cred_build_auth0_command(NULL, auth_scratch, sizeof(auth_scratch),
					   &auth_scratch_length), ULTRAWIDELOCK_CRED_AUTH_INVALID_ARGUMENT);
	struct ultrawidelock_cred_auth0_command bad_auth0 = auth0;
	bad_auth0.authentication_policy = 4;
	T_EQ("AUTH0 rejects bad policy",
	     ultrawidelock_cred_build_auth0_command(&bad_auth0, auth_scratch, sizeof(auth_scratch),
					   &auth_scratch_length), ULTRAWIDELOCK_CRED_AUTH_INVALID_ARGUMENT);
	T_EQ("AUTH0 rejects short buffer",
	     ultrawidelock_cred_build_auth0_command(&auth0, auth_scratch, 16, &auth_scratch_length),
	     ULTRAWIDELOCK_CRED_AUTH_BUFFER_TOO_SMALL);
	static const uint8_t vendor_extension[] = { 0x01, 0x02, 0x03, 0x04 };
	struct ultrawidelock_cred_auth0_command vendor_auth0 = auth0;
	...
	T_EQ("AUTH0 with vendor extension",
	     ultrawidelock_cred_build_auth0_command(&vendor_auth0, auth_scratch, sizeof(auth_scratch),
					   &auth_scratch_length), ULTRAWIDELOCK_CRED_AUTH_OK);
	T_EQ("AUTH0 vendor extension length", auth_scratch_length,
	     ULTRAWIDELOCK_CRED_AUTH0_STANDARD_COMMAND_SIZE + 6);
	vendor_auth0.vendor_extension_length = 130;
	T_EQ("AUTH0 rejects oversize vendor extension",
	     ultrawidelock_cred_build_auth0_command(&vendor_auth0, auth_scratch, sizeof(auth_scratch),
					   &auth_scratch_length), ULTRAWIDELOCK_CRED_AUTH_BUFFER_TOO_SMALL);

	T_EQ("AUTH0 response rejects null",
	     ultrawidelock_cred_parse_auth0_response(NULL, 0, 0, &auth0_response),
	     ULTRAWIDELOCK_CRED_AUTH_INVALID_ARGUMENT);
	static const uint8_t status_only[] = { 0x90 };
	T_EQ("AUTH0 response rejects short APDU",
	     ultrawidelock_cred_parse_auth0_response(status_only, sizeof(status_only), 0,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_INVALID_APDU);
	static const uint8_t auth_status_error[] = { 0x69, 0x85 };
	T_EQ("AUTH0 response rejects bad status",
	     ultrawidelock_cred_parse_auth0_response(auth_status_error, sizeof(auth_status_error), 0,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_STATUS_ERROR);
	static const uint8_t wrong_first_tag[] = { 0x41, 0x01, 0x00, 0x90, 0x00 };
	T_EQ("AUTH0 response rejects wrong first tag",
	     ultrawidelock_cred_parse_auth0_response(wrong_first_tag, sizeof(wrong_first_tag), 0,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_WRONG_CONTENT);

	uint8_t point[ULTRAWIDELOCK_CRED_PUBLIC_KEY_SIZE] = { 0x04 };
	uint8_t cryptogram[64] = { 0 };
	uint8_t fast_vendor[192];
	size_t fast_vendor_length = 0;
	T_EQ("assemble fast key TLV", ultrawidelock_cred_tlv_write(fast_vendor, sizeof(fast_vendor),
	     &fast_vendor_length, 0x86, point, sizeof(point)), ULTRAWIDELOCK_CRED_TLV_OK);
	T_EQ("assemble fast cryptogram TLV", ultrawidelock_cred_tlv_write(fast_vendor,
	     sizeof(fast_vendor), &fast_vendor_length, 0x9d, cryptogram, sizeof(cryptogram)),
	     ULTRAWIDELOCK_CRED_TLV_OK);
	T_EQ("assemble fast vendor TLV", ultrawidelock_cred_tlv_write(fast_vendor, sizeof(fast_vendor),
	...
	T_EQ("AUTH0 fast response with vendor extension",
	     ultrawidelock_cred_parse_auth0_response(fast_vendor, fast_vendor_length, 1,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_OK);
	T_EQ("AUTH0 vendor extension parsed", auth0_response.vendor_extension_length,
	     sizeof(vendor_extension));

	fast_vendor_length = 0;
	T_EQ("assemble dangling key TLV", ultrawidelock_cred_tlv_write(fast_vendor,
	...
	fast_vendor_length = 0;
	T_EQ("assemble key TLV again", ultrawidelock_cred_tlv_write(fast_vendor, sizeof(fast_vendor),
	     &fast_vendor_length, 0x86, point, sizeof(point)), ULTRAWIDELOCK_CRED_TLV_OK);
	static const uint8_t stray_byte = 0x00;
	T_EQ("assemble stray TLV", ultrawidelock_cred_tlv_write(fast_vendor, sizeof(fast_vendor),
	     &fast_vendor_length, 0x41, &stray_byte, 1), ULTRAWIDELOCK_CRED_TLV_OK);
	fast_vendor[fast_vendor_length++] = 0x90;
	fast_vendor[fast_vendor_length++] = 0x00;
	T_EQ("AUTH0 response rejects stray tag",
	     ultrawidelock_cred_parse_auth0_response(fast_vendor, fast_vendor_length, 0,
					     &auth0_response), ULTRAWIDELOCK_CRED_AUTH_WRONG_CONTENT);

	T_EQ("auth data rejects null reader",
	     ultrawidelock_cred_build_authentication_data(NULL, point, point, transaction_id, 0,
						 authentication_data),
	     ULTRAWIDELOCK_CRED_AUTH_INVALID_ARGUMENT);
	uint8_t compressed_point[ULTRAWIDELOCK_CRED_PUBLIC_KEY_SIZE] = { 0x02 };
	T_EQ("auth data rejects compressed key",
	     ultrawidelock_cred_build_authentication_data(reader_id, compressed_point, point,
						 transaction_id, 0, authentication_data),
	     ULTRAWIDELOCK_CRED_AUTH_INVALID_ARGUMENT);

	T_EQ("AUTH1 rejects null signature",
	     ultrawidelock_cred_build_auth1_command(0, NULL, auth_scratch, sizeof(auth_scratch),
					   &auth_scratch_length), ULTRAWIDELOCK_CRED_AUTH_INVALID_ARGUMENT);
	uint8_t certificate[100];
	memset(certificate, 0x33, sizeof(certificate));
	T_EQ("AUTH1 with certificate",
	     ultrawidelock_cred_build_auth1_command_ex(0, signature, certificate, sizeof(certificate),
					      auth_scratch, sizeof(auth_scratch),
					      &auth_scratch_length), ULTRAWIDELOCK_CRED_AUTH_OK);
	T_EQ("AUTH1 certificate length", auth_scratch_length,
	     ULTRAWIDELOCK_CRED_AUTH1_COMMAND_SIZE + 102);
