/* CBMC harness — ultrawidelock_uwb_msg TLV parser memory safety.
 *
 ...
 * at ultrawidelock_uwb_msg_parser.c:24 (the lone-trailing-byte over-read) is what makes
 * it pass. */
#include <stddef.h>
#include <stdint.h>
...
void harness(void)
{
	...
	__CPROVER_assume(size >= ULTRAWIDELOCK_HEADER_LENGTH && size <= MSG_MAX);

	struct ultrawidelock_uwb_msg_parser parser = {
		.length = size,
	...
	while ((attr = ultrawidelock_uwb_msg_next_attribute(&parser)) != NULL) {
		uint8_t v8;
		uint16_t v16;
		uint32_t v32;
		uint64_t v64;

		(void)ultrawidelock_uwb_msg_read_u8(attr, "", &v8);
		(void)ultrawidelock_uwb_msg_read_u16(attr, "", &v16);
		(void)ultrawidelock_uwb_msg_read_u32(attr, "", &v32);
		(void)ultrawidelock_uwb_msg_read_u64(attr, "", &v64);
	}
}
