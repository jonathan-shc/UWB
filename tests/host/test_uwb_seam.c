 ...
 * ultrawidelock_uwb_arm_rx and ultrawidelock_uwb_set_sts_iv as real functions, which is precisely
...
static void test_uwb_seam(void)
{
	...
	T_EQ("arm rx returns what the radio returned", ultrawidelock_uwb_arm_rx(0), 0L);
	...
	T_EQ("a radio failure is propagated", ultrawidelock_uwb_arm_rx(2), -1L);
	...
	T_EQ("configure returns what the radio returned", ultrawidelock_uwb_configure_phy(&config), 0L);
	...
	T_EQ("a configure failure is propagated", ultrawidelock_uwb_configure_phy(&config), -1L);
