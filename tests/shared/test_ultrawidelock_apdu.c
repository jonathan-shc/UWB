...
int main(void)
{
	...
	okc("build.ok", ultrawidelock_apdu_build_auth0(0x02, 0x00, 0x0100, pub, txid, rid, out,
					       sizeof(out), &n) == 0);
	okc("len==129", n == 129);
	...
	okc("build.ok", ultrawidelock_apdu_build_auth1(0x01, sig, out, sizeof(out), &n) == 0);
	...
	okc("reader.ok", ultrawidelock_apdu_build_authdata(ULTRAWIDELOCK_AUTH_READER, rid, pubx, pubx,
	...
	ultrawidelock_apdu_build_authdata(ULTRAWIDELOCK_AUTH_DEVICE, rid, pubx, pubx, txid, out,
				  sizeof(out), &n);
	veq("device.usage", out + n - 6, 6, "93044e887b4c");

	printf("\n== EXCHANGE + 0x98 trigger ==\n");
	okc("ursk-ready.ok",
	    ultrawidelock_apdu_build_exchange(0, 0, 1, out, sizeof(out), &n) == 0);
	veq("ursk-ready.bytes", out, n, "9800");
	ultrawidelock_apdu_build_exchange(1, 0x0005, 1, out, sizeof(out), &n);
	veq("status+ready.bytes", out, n, "970200059800");

	printf("\n== AUTH0 standard path: 1-byte phase (41 01 00) + APDU wrap + SW strip ==\n");
	okc("build.phase0", ultrawidelock_apdu_build_auth0(0x00, 0x01, 0x0100, pub, txid, rid, out,
						   sizeof(out), &n) == 0);
	...
	okc("build.fast", ultrawidelock_apdu_build_auth0(0x01, 0x01, 0x0100, pub, txid, rid, out,
						 sizeof(out), &n) == 0);
	okc("phase.fastbit", out[0] == 0x41 && out[1] == 0x01 && out[2] == 0x01);
	{
		...
		okc("wrap.ok", ultrawidelock_apdu_wrap(ULTRAWIDELOCK_INS_AUTH0, out, n, apdu, sizeof(apdu),
					       &alen) == 0);
	...
	}
	{
		...
		okc("strip.sw9000", ultrawidelock_apdu_strip_sw(rb, &rl, &sw) == 0 && rl == 2 &&
					    sw == 0x9000);
		size_t one = 1;

		okc("strip.tooshort", ultrawidelock_apdu_strip_sw(rb, &one, &sw) < 0);
	}

	printf("\n== L2CAP envelope ==\n");
	uint8_t pl[3] = { 0xAA, 0xBB, 0xCC };

	ultrawidelock_ble_frame(0x00, ULTRAWIDELOCK_INS_AUTH0, pl, 3, out, sizeof(out), &n);
	...
	okc("unframe.ok",
	    ultrawidelock_ble_unframe(out, n, &ty, &op, &pp, &ppl) == 0 && ty == 0 &&
	...
	ultrawidelock_tlv_w_init(&w, rbuf, sizeof(rbuf));
	ultrawidelock_tlv_put(&w, 0x86, pub, 65);
	ultrawidelock_tlv_put(&w, 0x9D, sig, 64);
	ultrawidelock_tlv_w_finish(&w, &rlen);
	struct ultrawidelock_auth0_response a0;

	okc("auth0resp.parse", ultrawidelock_apdu_parse_auth0_response(rbuf, rlen, &a0) == 0);
	...
	okc("auth0resp.no-crypto",
	    ultrawidelock_apdu_parse_auth0_response(rbuf, rlen, &a0) == 0 && a0.have_cryptogram == 0);
	/* missing mandatory 0x86 -> reject */
	ultrawidelock_tlv_w_init(&w, rbuf, sizeof(rbuf));
	ultrawidelock_tlv_put(&w, 0x9D, sig, 64);
	ultrawidelock_tlv_w_finish(&w, &rlen);
	okc("auth0resp.reject-no86",
	    ultrawidelock_apdu_parse_auth0_response(rbuf, rlen, &a0) < 0);

	/* AUTH1Response: 5A 41 <65> 9E 40 <64> */
	ultrawidelock_tlv_w_init(&w, rbuf, sizeof(rbuf));
	ultrawidelock_tlv_put(&w, 0x5A, pub, 65);
	ultrawidelock_tlv_put(&w, 0x9E, sig, 64);
	ultrawidelock_tlv_w_finish(&w, &rlen);
	struct ultrawidelock_auth1_response a1;

	okc("auth1resp.parse", ultrawidelock_apdu_parse_auth1_response(rbuf, rlen, &a1) == 0);
	...
	ultrawidelock_tlv_w_init(&w, bigout, sizeof(bigout));
	ultrawidelock_tlv_put(&w, 0xB2, big, 200);
	ultrawidelock_tlv_w_finish(&w, &rlen);
	okc("longform.hdr", bigout[0] == 0xB2 && bigout[1] == 0x81 && bigout[2] == 200);
	const uint8_t *fv;
	size_t fl;

	okc("longform.find", ultrawidelock_tlv_find(bigout, rlen, 0xB2, &fv, &fl) == 0 &&
	...
	printf("\n== error paths: writer overflow / reader forms / wrap / envelope ==\n");
	{
		...
		ultrawidelock_tlv_w_init(&w, tiny, sizeof(tiny));
		ultrawidelock_tlv_put(&w, 0x9E, sig, 64);
		okc("writer.overflow", ultrawidelock_tlv_w_finish(&w, &tn) < 0);
	}
	{
		...
		ultrawidelock_tlv_w_init(&w, hugebuf, sizeof(hugebuf));
		ultrawidelock_tlv_put(&w, 0xB3, hugev, sizeof(hugev));
		okc("longform82.write", ultrawidelock_tlv_w_finish(&w, &hn) == 0 && hn == 4 + 300 &&
					       hugebuf[1] == 0x82 && hugebuf[2] == 0x01 &&
					       hugebuf[3] == 0x2C);
		okc("longform82.find", ultrawidelock_tlv_find(hugebuf, hn, 0xB3, &hv, &hl) == 0 &&
	...
	}
	{
		...
		okc("find.trunc81", ultrawidelock_tlv_find(trunc81, sizeof(trunc81), 0x99, &tv, &tl) < 0);
		okc("find.trunc82", ultrawidelock_tlv_find(trunc82, sizeof(trunc82), 0x99, &tv, &tl) < 0);
		okc("find.badlenform", ultrawidelock_tlv_find(badl0, sizeof(badl0), 0x99, &tv, &tl) < 0);
		okc("find.overrun", ultrawidelock_tlv_find(overrun, sizeof(overrun), 0x99, &tv, &tl) < 0);
	}
	{
		...
		okc("wrap.null", ultrawidelock_apdu_wrap(0x80, NULL, 10, apdu, sizeof(apdu), &alen) < 0);
		okc("wrap.empty", ultrawidelock_apdu_wrap(0x80, big, 0, apdu, sizeof(apdu), &alen) < 0);
		okc("wrap.too-long",
		    ultrawidelock_apdu_wrap(0x80, big, 256, apdu, sizeof(apdu), &alen) < 0);
		okc("wrap.cap", ultrawidelock_apdu_wrap(0x80, big, 100, apdu, 50, &alen) < 0);
	}
	{
		/* AUTH1Response: missing signature rejected; 0x91 bitmap ignored */
		struct ultrawidelock_auth1_response ax;

		ultrawidelock_tlv_w_init(&w, rbuf, sizeof(rbuf));
		ultrawidelock_tlv_put(&w, 0x5A, pub, 65);
		ultrawidelock_tlv_w_finish(&w, &rlen);
		okc("auth1resp.reject-nosig",
		    ultrawidelock_apdu_parse_auth1_response(rbuf, rlen, &ax) < 0);
		ultrawidelock_tlv_w_init(&w, rbuf, sizeof(rbuf));
		ultrawidelock_tlv_put(&w, 0x9E, sig, 64);
		ultrawidelock_tlv_put_u16(&w, 0x91, 0x0003);
		ultrawidelock_tlv_w_finish(&w, &rlen);
	...
	}
	{
		...
		okc("frame.too-big",
		    ultrawidelock_ble_frame(0, 0x80, pl, 0x10000u, fout, sizeof(fout), &fn) < 0);
		okc("frame.cap", ultrawidelock_ble_frame(0, 0x80, pl, 3, fout, 4, &fn) < 0);
		okc("unframe.short",
		    ultrawidelock_ble_unframe(shortenv, sizeof(shortenv), &ty, &op, &pp2, &pl2) < 0);
		okc("unframe.lying-len",
		    ultrawidelock_ble_unframe(lieenv, sizeof(lieenv), &ty, &op, &pp2, &pl2) < 0);
	}
