 ...
 * ports/esp32/components/aliro_ble/aliro_ble.c), and the salt check is pinned to
 * the reader's own hardcoded BleSK salt — the 01 00 01 00 that aliro_reader.c
...
static void make_adv(uint8_t out[ALIRO_BLE_CENTRAL_SVC_DATA_LEN], const uint8_t reader_id[32],
		     uint32_t expiry)
{
	uint8_t *p = out;
	...
	*p++ = (uint8_t)expiry;
	*p++ = 0x00u; /* reserved */
	for (unsigned i = 0; i < ALIRO_ADVTAG_LEN; i++) {
...
static void test_adv(void)
{
	...
	uint8_t svc[ALIRO_BLE_CENTRAL_SVC_DATA_LEN];
	struct aliro_ble_central_adv adv;

	make_adv(svc, reader_id, 0x6890ABCDu);
	T_OK("adv.parse", aliro_ble_central_parse_adv(svc, sizeof(svc), &adv) == 0);
	...
	T_OK("adv.tag[0]", adv.tag[0] == 0xA0u && adv.tag[ALIRO_ADVTAG_LEN - 1u] == 0xA6u);

	T_OK("adv.matches-our-reader", aliro_ble_central_adv_matches(&adv, reader_id) == 1);
	...
	other[3] ^= 0x01u;
	T_OK("adv.rejects-other-reader", aliro_ble_central_adv_matches(&adv, other) == 0);

	/* a byte outside both truncated spans is invisible to the match, by design */
	memcpy(other, reader_id, 32);
	other[20] ^= 0x01u;
	T_OK("adv.match-ignores-untruncated-bytes",
	     aliro_ble_central_adv_matches(&adv, other) == 1);

	/* no-clock readers advertise the sentinel expiry */
	make_adv(svc, reader_id, 0xFFFFFFFFu);
	T_OK("adv.expiry-unavailable",
	     aliro_ble_central_parse_adv(svc, sizeof(svc), &adv) == 0 && adv.expiry == 0xFFFFFFFFu);

	printf("\n== advert decode rejects malformed input ==\n");
	T_OK("adv.short", aliro_ble_central_parse_adv(svc, sizeof(svc) - 1u, &adv) == -1);
	T_OK("adv.long", aliro_ble_central_parse_adv(svc, sizeof(svc) + 1u, &adv) == -1);
	svc[0] = 0xF1u; /* not the Aliro service */
	T_OK("adv.wrong-uuid", aliro_ble_central_parse_adv(svc, sizeof(svc), &adv) == -1);
}
...
static void test_read_payload(void)
{
	...
	T_OK("read.parse", aliro_ble_central_parse_read_payload(buf, n, &peer) == 0);
	...
	T_OK("read.short", aliro_ble_central_parse_read_payload(buf, 2u, &peer) == -1);
	T_OK("read.truncated-features",
	     aliro_ble_central_parse_read_payload(buf, n - 1u, &peer) == -1);

	uint8_t bad[64];

	memcpy(bad, buf, n);
	bad[2] = 3u; /* odd versions length: versions are 2 bytes each */
	T_OK("read.odd-versions-len", aliro_ble_central_parse_read_payload(bad, n, &peer) == -1);

	memcpy(bad, buf, n);
	bad[2] = (uint8_t)((ALIRO_BLE_CENTRAL_MAX_VERSIONS + 1u) * 2u);
	T_OK("read.too-many-versions", aliro_ble_central_parse_read_payload(bad, n, &peer) == -1);

	memcpy(bad, buf, n);
	bad[2] = 200u; /* versions length overruns the buffer */
	T_OK("read.versions-len-overrun",
	     aliro_ble_central_parse_read_payload(bad, n, &peer) == -1);

	memcpy(bad, buf, n);
	bad[n - 2u] = 0u; /* features length of zero */
	T_OK("read.zero-features-len", aliro_ble_central_parse_read_payload(bad, n, &peer) == -1);
}
...
static void test_blesk_salt(void)
{
	 ...
	 * must be byte-identical to the 01 00 01 00 that aliro_reader.c builds from
	 * k_proto_versions, which is what the already-proven aliro_dev_blesk_init
	 * call sites hardcode. */
	static const uint8_t k_reader_salt_v1[] = {0x01, 0x00, 0x01, 0x00};
	const uint16_t v1[] = {0x0100u};

	n = make_read_payload(buf, 0x0080u, v1, 1u, 0x05u);
	T_OK("salt.parse", aliro_ble_central_parse_read_payload(buf, n, &peer) == 0);
	T_OK("salt.build",
	     aliro_ble_central_blesk_salt(&peer, 0x0100u, salt, sizeof(salt), &sl) == 0);
	...
	T_OK("salt.parse-multi", aliro_ble_central_parse_read_payload(buf, n, &peer) == 0);
	T_OK("salt.build-multi",
	     aliro_ble_central_blesk_salt(&peer, 0x0200u, salt, sizeof(salt), &sl) == 0);
	...
	T_OK("salt.cap-too-small",
	     aliro_ble_central_blesk_salt(&peer, 0x0100u, salt, 5u, &sl) == -1);

	struct aliro_ble_central_peer empty;

	memset(&empty, 0, sizeof(empty));
	T_OK("salt.no-versions",
	     aliro_ble_central_blesk_salt(&empty, 0x0100u, salt, sizeof(salt), &sl) == -1);
}

int main(void)
{
	printf("== aliro_ble_central: device-side BLE transport decoders ==\n");
	test_adv();
	test_read_payload();
	test_blesk_salt();
