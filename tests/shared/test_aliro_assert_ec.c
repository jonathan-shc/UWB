...
int main(void)
{
	struct ultrawidelock_assert_ec_priv priv;
	struct ultrawidelock_assert_ec_pub pub;
	uint8_t p[ULTRAWIDELOCK_P256_SCALAR];
	uint8_t q[ULTRAWIDELOCK_P256_POINT];

	check("prim_init", ultrawidelock_prim_init() == 0);
	check("keygen", ultrawidelock_ec_p256_keygen(p, q) == 0);
	...
	check("build", ultrawidelock_assert_build_p256(ultrawidelock_assert_ec_sign, &priv, &a,
						       wire, sizeof(wire), &wlen) == 0);
	check("build.len", wlen == ULTRAWIDELOCK_ASSERT_WIRE_P256);
	check("build.alg", wire[3] == ULTRAWIDELOCK_ASSERT_ALG_ECDSA_P256);

	struct ultrawidelock_assert out;

	check("verify", ultrawidelock_assert_verify_p256(ultrawidelock_assert_ec_verify, &pub, wire,
							 wlen, a.nonce, a.cred_id, 100, 0,
							 &out) == ULTRAWIDELOCK_ASSERT_OK);
	 ...
	 * Deliberately NOT a second keygen call -- ultrawidelock_random in the host double
	...
	struct ultrawidelock_assert_ec_pub other;
	uint8_t p2[ULTRAWIDELOCK_P256_SCALAR];
	uint8_t q2[ULTRAWIDELOCK_P256_POINT];

	memcpy(p2, p, sizeof(p2));
	p2[0] ^= 0xFF;
	check("pub_from_priv2", ultrawidelock_ec_p256_pub_from_priv(p2, q2) == 0);
	check("keys_differ", memcmp(q2, q, sizeof(q2)) != 0);
	memcpy(other.q, q2, sizeof(q2));
	check("verify.wrong_key",
	      ultrawidelock_assert_verify_p256(ultrawidelock_assert_ec_verify, &other, wire, wlen,
					       a.nonce, a.cred_id, 100, 0,
					       &out) == ULTRAWIDELOCK_ASSERT_E_MAC);

	/* Tamper the signed prefix. */
	uint8_t tam[ULTRAWIDELOCK_ASSERT_WIRE_P256];

	memcpy(tam, wire, sizeof(tam));
	tam[30] ^= 0x01; /* distance_cm low byte */
	check("verify.tampered",
	      ultrawidelock_assert_verify_p256(ultrawidelock_assert_ec_verify, &pub, tam,
					       sizeof(tam), a.nonce, a.cred_id, 100, 0,
					       &out) == ULTRAWIDELOCK_ASSERT_E_MAC);

	/* NULL key material is a wiring bug: fail, do not dereference. */
	check("sign.null_ctx", ultrawidelock_assert_ec_sign(NULL, wire, 8, tam) != 0);
	check("verify.null_ctx", ultrawidelock_assert_ec_verify(NULL, wire, 8, tam) != 0);
	check("build.null_key",
	      ultrawidelock_assert_build_p256(ultrawidelock_assert_ec_sign, NULL, &a, wire,
					      sizeof(wire), &wlen) == -1);

	printf("  ultrawidelock_assert_ec: %d checks, %d failed\n", total, fails);
	return fails == 0 ? 0 : 1;
}
