...
printf '\n── cdk size · the Discord notifier\n'

# Two things are worth pinning here. The silence rule, because a bot that posts
# on every no-op gets muted and a muted bot is worse than none. And the
# sanitising, because on a fork pull request every byte of the report -- symbol
# names included -- was produced by that fork, and those names go into a message
# posted in somebody's server.
python3 - "$TMP" <<'PY'
import importlib.util
import json
import os
...
tmp = sys.argv[1]
spec = importlib.util.spec_from_file_location("notify", "scripts/cdk-size-notify.py")
...
def rep(ram_used, symbols=None, conf=SHIP):
    return {
        "commit": "0" * 40,
        "config": {"board": "decawave_dwm3001cdk", "image": "firmware",
                   "extra_conf_file": conf, "ncs_version": "3.3.0",
        ...
        "regions": {
            "RAM": {"origin": 0x20000000, "size": size, "used": ram_used,
                    "free": size - ram_used, "pct": 92.5},
            "FLASH": {"origin": 0xA200, "size": 433664, "used": 417308,
                      "free": 16356, "pct": 96.23},
        },
        "symbols": symbols or {},
    }
...
baseline = rep(121236)
baseline["gate"] = {"ram_free_floor": 4096, "flash_free_floor": 8192,
                    "ram_delta_cap": 2048, "flash_delta_cap": 4096}
doc = {"schema": 2, "primary": "thread+release+smp+lto",
       "baselines": {"thread+release+smp+lto": baseline}}

# 1. a byte-identical run says nothing at all
embed, why = n.build(rep(121236), doc, "", 1)
if embed is None and why:
    print("  ok    an unchanged passing run posts nothing")
else:
    print("  FAIL  an unchanged run would have posted")
    rc = 1

# 2. anything that moved does speak
embed, _ = n.build(rep(121736), doc, "", 1)
if embed is not None and "+500" in json.dumps(embed):
    print("  ok    a +500 B move is reported")
else:
    print("  FAIL  a +500 B move was not reported")
    rc = 1

# 3. the threshold can raise the bar without silencing a block
embed, why = n.build(rep(121736), doc, "", 4096)
if embed is None:
    print("  ok    CDK_SIZE_NOTIFY_MIN can mute a small move")
else:
    print("  FAIL  the threshold did not mute a small move")
    rc = 1

# ...but a floor breach is never muted, however high the threshold.
embed, _ = n.build(rep(size - 100), doc, "", 10 ** 9)
if embed is not None and "blocked" in embed["title"]:
    print("  ok    a blocked run speaks at any threshold")
else:
    print("  FAIL  a blocked run was muted by the threshold")
    rc = 1

# 4. hostile symbol names cannot ping, link, or escape the code span
hostile = rep(126236, {
    "@everyone": 9000,
    "`](https://evil.example)": 8000,
    "<@&1234567890>": 7000,
})
embed, _ = n.build(hostile, doc, "", 1)
blob = json.dumps(embed)
for bad, what in [("@everyone", "an everyone mention"),
                  ("<@&", "a role mention"),
                  ("](http", "a markdown link")]:
    if bad in blob:
        print(f"  FAIL  {what} survived sanitising")
        rc = 1
    else:
        print(f"  ok    {what} is stripped")

# 5. and a legitimate C symbol survives INTACT -- the underscores matter,
# because an unsearchable name makes the top-mover list useless.
embed, _ = n.build(rep(124244, {"cdk_size_gate_probe": 3000}), doc, "", 1)
if "cdk_size_gate_probe" in json.dumps(embed):
    print("  ok    a real C symbol keeps its underscores")
else:
    print("  FAIL  a real C symbol was mangled")
    rc = 1

# 6. the /github and /slack endpoint suffixes are stripped. A repository that
# already forwards pull-request events to Discord has the /github form in its
# settings, and that is the URL somebody copies when asked for "the webhook";
# posting an embed there fails in a way that reads like a malformed embed.
base = "https://discord.com/api/webhooks/123/abcDEF"
for raw in (base, base + "/github", base + "/github/", base + "/slack",
            base + "/GitHub", "  " + base + "  "):
    got = n.normalise_webhook(raw)
    if got == base:
        print(f"  ok    webhook URL normalised: ...{raw[-12:]!r}")
    else:
        print(f"  FAIL  {raw!r} -> {got}")
        rc = 1

# 7. an unrecorded configuration is reported as such, never as a delta
embed, _ = n.build(rep(121236, conf="overlay-thread.conf;overlay-lto.conf"), doc, "", 1)
if embed is not None and "no baseline" in embed["title"]:
    print("  ok    an unrecorded configuration is named, not compared")
else:
    print("  FAIL  an unrecorded configuration was not handled")
    rc = 1
...
# No webhook configured must be a clean no-op, not a red X: every fork of this
# repository is in that state and none of them asked for a notifier.
(unset DISCORD_WEBHOOK; python3 "$ROOT/scripts/cdk-size-notify.py" \
	--current "$TMP/ship.json" --baseline "$TMP/multi.json" >/dev/null 2>&1)
expect_exit 0 "$?" "a missing DISCORD_WEBHOOK is a no-op, not a failure"
