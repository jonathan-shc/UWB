// nRF5340DK on-target self-test for the credential device (initiator) EC path: a
...
/*
 * nRF5340 entry for the credential device (initiator) EC self-test. Initialises the
...
/**
 * Run the on-target credential device EC self-test: initialize PSA, run the device self-test, and
 * print PASS or FAIL with return code.
 */
int main(void)
{
	printf("\n=== on-target credential device EC self-test (nRF5340, nrf_security PSA) ===\n");
