...
export function isAwakeNow(
	utcOffsetMinutes: number,
	awakeStartHour: number,
	awakeEndHour: number,
	nowMs: number = Date.now(),
): boolean {
	if (awakeStartHour === awakeEndHour) return true;

	const utcMinutesOfDay = (Math.floor(nowMs / 60_000) + utcOffsetMinutes) % 1440;
	const localMinutesOfDay = ((utcMinutesOfDay % 1440) + 1440) % 1440;
	const localHour = Math.floor(localMinutesOfDay / 60);

	if (awakeStartHour < awakeEndHour) {
		return localHour >= awakeStartHour && localHour <= awakeEndHour;
	}
	// Overnight window, e.g. "22-6": awake from 22:00 through 23:59, then
	// again from 00:00 through 06:59.
	return localHour >= awakeStartHour || localHour <= awakeEndHour;
}
...
export function nextWakeUnixMs(utcOffsetMinutes: number, awakeStartHour: number, nowMs: number = Date.now()): number {
	...
	const localMinutesOfDay = ((nowMinute + utcOffsetMinutes) % 1440 + 1440) % 1440;
	const targetMinutesOfDay = awakeStartHour * 60;
	let deltaMinutes = targetMinutesOfDay - localMinutesOfDay;
	if (deltaMinutes <= 0) deltaMinutes += 1440;
	return (nowMinute + deltaMinutes) * 60_000;
}
