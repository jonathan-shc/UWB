...
apply_to "$ADDON"                 "$P/custom_impl-uwb.patch" "$P/crypto-timesync-tap.patch" "$P/pretty-shell.patch" "$P/console-quiet-flood.patch" "$P/aliro-doc-time-ratchet.patch" "$P/aliro-time-persist.patch"
apply_to "$WS/nrf"                "$P/nrf-flashfit-dfu-guards.patch"
apply_to "$WS/modules/lib/matter" "$P/matter-ble-multi-identity.patch"
echo "    ✓ pristine upstream + 8 patches (add-on ×6, nrf, matter)"
