...
NET_BUF_POOL_FIXED_DEFINE(s_dfu_tx_pool, 1, BT_L2CAP_SDU_BUF_SIZE(WOZ_DFU_RSP_MAX),
			  CONFIG_BT_CONN_TX_USER_DATA_SIZE, NULL);
...
static int dfu_recv(struct bt_l2cap_chan *chan, struct net_buf *buf)
{
	...
	(void)woz_dfu_rx_frame(buf->data, buf->len, rsp, &rsp_len);
	if (rsp_len == 0U) {
		return 0;
	}
 ...
 * reaches the handler and is refused inside it, with WOZ_DFU_ERR_CLOSED. That
			      ...
			      uint8_t flags)
{
	...
	(void)woz_dfu_rx_frame(buf, len, rsp, &rsp_len);
	if (rsp_len > 0U) {
...
{
	ARG_UNUSED(work);
	woz_dfu_window_open(CONFIG_WOZ_DFU_WINDOW_MS);
}
